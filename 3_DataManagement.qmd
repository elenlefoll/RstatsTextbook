...
check_hint("This screenshot was taken on a computer running macOS as its operating system.", hint_title = "🐭 Click on the mouse for a hint.")
```
...
right = "That's right!",
wrong = "No, not quite.")
check_hint("From the `Rscript` folder, you will need to go \"back up the path\" twice: once to get to the course folder `2024_SoSe_Stats` and a second time to get to the `UzK` folder, before you can move to the `2023_SoSe_CADS` folder. Going back up the path is achieved with `../`.", hint_title = "😇 Hover for a hint", type = "onmouseover")
```
