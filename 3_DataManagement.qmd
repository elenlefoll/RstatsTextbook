...
# Project organisation {#sec-DataManagement}
...
Data management is hardly a "hot" topic that people like to dwell on. That's a shame because good file management is absolutely central to be able to conduct research and poor file management has the potential to seriously 'spice things up'... but not in a good way! `r emoji("hot_pepper")`️ Whether you are working on a short course assignment, your Master's or PhD thesis, or as part of a large research project team: research-related files must be named appropriately and safely stored in meaningful places.
...
Two sets of filenames, one clearly better than the other.
:::
...
Finally, it is worth noting that filenames cannot be infinitely long! The maximum length of a filename depends on the operating system and the application that you use[^3_datamanagement-1] but, as a rule of thumb, if you can display the entire filename in a reasonably sized Finder window (on macOS) or File Explorer window (on Windows), its length is unquestionably both human- and computer-friendly.
...
To help us remember the different, systematic ways to use letter case, hyphens, and underscores in file and variable naming conventions, these patterns have fun names.

![[Artwork CC BY 4.0 \@allison_horst](https://twitter.com/allison_horst))](images/AHorst_InThatCase.png){fig-alt="Cartoon representations of common cases in coding. A snake screams \"SCREAMING_SNAKE_CASE\" into the face of a camel (wearing ear muffs) with \"camelCase\" written along its back. Vegetables on a skewer spell out \"kebab-case\" (words on a skewer). A mellow, happy looking snake has text \"snake_case\" along it."}

[**Q3.1**]{style="color:green;"} In which case is this filename? `my_first_file_name.R`
...
[**Q3.2**]{style="color:green;"} Why is this filename problematic? `MyDocument final.1a.docx`

```{r}
#| echo: false
#| label: "Q3.2"

check_question(c("Spaces in filename", "Lack of clarity", "Mixed capitalisation", "Use of special character other than _ or -"), options = c("Spaces in filename", "Lack of clarity", "Mixed capitalisation", "Use of special character other than _ or -"), type = "checkbox",
random_answer_order = TRUE,
button_label = "Check answer", q_id = "Q3.2",
right = "That's right! This filename is problematic due to the fact that it contains spaces, that it is unclear what 'MyDocument' and 'final' mean, and the use of the dot, which should only be used as part of the file extension. While mixed capitalisation will not cause any errors, it is not good filenaming practice and should therefore be avoided." ,
wrong = "Not quite. There are a lot of issues with this filename!")
```

[**Q3.3**]{style="color:green;"} Which of these filenames are both human-friendly and computer-friendly?
...
wrong = "Not quite. Here's a hint: There are two human-friendly and computer-friendly filenames in this list.")

```
:::
::::

It is also important to ensure that filenames are easily sortable. If you have a series of files that document a process, consider beginning each filename with a number that correspond to the order of the process, e.g. `01_DataPreparation.R`, `02_DataAnnotation.R`, `03_AnnotationEvaluation.R`. Left-padding the numbers with one or more `0` will mean that the files are sorted numerically, even when files are listed alphabetically (see @fig-FileNamesLeftPaddedNumbers).

::: {#fig-NumberPadding layout="[[40,-1,40], [100]]"}
![Filenames without additional zeros numbers](images/FileNamesLexicographic.png){#fig-FileNamesNaturalNumbers fig-alt="List of filenames without left-padded numbers so that, when sorted alphabetically, these files are not ordered numerically."}

![Filenames with left-padded numbers](images/FileNamesPaddedNumbers.png){#fig-FileNamesLeftPaddedNumbers fig-alt="List of filenames with left-padded numbers that are ordered alphabetically. The files are ordered in a clear and organised manner."}

Why left-padding filenames is good file naming practice.
:::

It is sometimes useful to include the date in filenames. However, many date formats are not easily sortable (see @fig-FileNamesDateFormatUnordered). Formatting dates using the 'YYYY-MM-DD' format as in @fig-FileNamesDateFormatOrdered will allow you to easily sort your files in chronological order.

::: {#fig-Dates layout-ncol="2"}
![Filenames with a non-ordered date format](images/FileNamesDateFormatUnordered.png){#fig-FileNamesDateFormatUnordered fig-alt="A finder window showing the files pane with filenames in an unordered date format. The files cannot be ordered meaningfully based on the date in the filename."}

![Filenames with an ordered date format](images/FileNamesDateFormatOrdered.png){#fig-FileNamesDateFormatOrdered fig-alt="A finder window showing the files pane with filenames in an ordered date format (YYYY-MM-DD). The files can be meaningfully ordered based on the date in the filename."}

Why using the YYYY-MM-DD is good filenaming practice.
:::

YYYY-MM-DD is the internationally recommended standard for dates ([ISO 8601](https://www.iso.org/iso-8601-date-and-time-format.html)). It provides a clear, computer-readable representation of dates that avoids the confusion that can arise from different cultures using different day-month conventions (e.g. 12/13 can mean both 12 December and 13, November).
...
-   on Windows/Linux: Right-click on the file (in some older Windows versions, you may also need to press the {{< kbd Shift >}} key). Among the options presented to you, click on the one to copy the file path (e.g. "Copy as path" or similar in the language of your operating system).

-   on macOS: Right-click on the file and then press the {{< kbd Option>}} key on your keyboard. Pressing down this key will change the options you are given after having right-clicked. One of these options should now be "Copy ... as Pathname" (or something equivalent in the language of your operating system). Click on this option.

Then, open any text-editing programme (e.g. LibreOffice Writer, Microsoft Word, NotePad++, TextEdit) and use the shortcut {{< kbd mac=Cmd-V win=Ctrl-V linux=Ctrl-V >}} to paste your file's path in the empty document. If you are on Windows, your path should have backslashes, whereas if you are on Linux or macOS, your path should have forward slashes.
...
If you want to find out more about this study, two of my linguistics M.A. students, Hannah Boxleitner & Beyhan Aliy reproduced @teraiEffectsLearningDirection2021's analyses in `R` and documented every step in a [case-study chapter](https://elenlefoll.github.io/RstatsTextbook/B_CaseStudies/Hannah-Beyhan/Retrieval.html) that can be found in the online version of this textbook.
:::
::::

## Backing up data: 'Fire safety' measures in the digital kitchen `r emoji("fire_extinguisher")`️ {#sec-BackupData}
...
The kitchen workflow metaphor ([artwork CC BY 4.0 \@allison_horst](https://twitter.com/allison_horst)).
:::
...
-   The University of Queensland Library. 2023. Work with Data and Files. The University of Queensland. Open Educational Resource. <https://uq.pressbooks.pub/digital-essentials-data-and-files/>.
...
It's time to complete this chapter's [tasks and quizzes](https://elenlefoll.github.io/RstatsTextbook/quizzes/3_quiz.html)! Good luck! `r emoji("four_leaf_clover")`
:::
...
If that's the case, you are now all set to install `R` and RStudio in @sec-Installing and learn how to get started in `R` in @sec-GettingStaRted!
