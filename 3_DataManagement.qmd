...
Finally, it is worth noting that file names cannot be infinitely long! The maximum length of a file name depends on the operating system and the application that you use[^datamanagement-1] but, as a rule of thumb, if you can display the entire file name in a reasonably sized Finder window (on macOS) or File Explorer window (on Windows), its length is unquestionably both human- and computer-friendly.

[^datamanagement-1]: For example, many Windows applications have a maximum file path length of 260 characters[@alvinashcraftMaximumPathLength2022].
