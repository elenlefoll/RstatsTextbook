...
Our starting point for this chapter is the wrangled combined dataset that we created and saved in @sec-DataWrangling. Follow the instructions in @sec-filter to create this `R` object or download `Dabrowska2019.zip` from [the textbook's GitHub repository](https://github.com/elenlefoll/RstatsTextbook/blob/main/Dabrowska2019.zip){.uri}. To get started, unzip the file and then double-click on the `Dabrowska2019.Rproj` file to ensure that RStudio opens up from the correct project directory.
:::
