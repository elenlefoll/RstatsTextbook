...
check_question(c("The bar plot because humans are better at distinguishing small differences in lengths than in angles.",
               ...
               button_label = "Check answer",
               random_answer_order = TRUE,
               right = "Correct! Bar charts are better suited than pie charts because humans are much better at estimating lengths than angles (see @sec-coordinates), and the labels are immediately next to the corresponding bars here.",
               wrong = "No, try again.")
...
[**d.**]{style="color:green;"} Transform the pie chart that you just created in [c.]{style="color:green;"} to make it look like @fig-\*\*. To achieve this, wrangle the data *before* piping it into the `data` argument of the `ggplot()` function. Which {tidyverse} function can you use to rename the labels?
...
@fig-SillyDensityPlot is an example of a density plot with some highly customised aesthetics. It goes without saying that, just because you *can* customise many aesthetic aspects of a `geom_` layer, it doesn't necessarily mean that it's a good idea to do so! 🙃
...
[**Q10.3**]{style="color:green;"} Create a boxplot to compare how participants in different occupational groups (`OccupGroup`) performed on the English grammar test. Which part of the code used to produce @fig-GrammarBoxPlotGroup <!--# @Vishar this should be the boxplot just above] do you need to modify to achieve this? -->
...
[**Q10.4**]{style="color:green;"} The code below was used to create @fig-OccupGroupBoxPlot, except that the arguments of the `aes()` function have been deleted. Which data mappings were specified inside the `aes()` function to produce @fig-OccupGroupBoxPlot?

```{r}
#| eval: false
#| label: "fig-OccupGroupBoxPlot"
