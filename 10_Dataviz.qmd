...
:::::: content-hidden
### Second attempt for PDF

This currently does not work as expected. Code inspired by this discussion: https://github.com/quarto-dev/quarto-cli/issues/13410.
...
#| fig-alt: "Barplot of participant ages in the Dabrowska study."
ggplot(data = Dabrowska.data,
       mapping = aes(Age)) +
  geom_bar() +
  scale_x_continuous() +
...
Barplots are best suited for categorical data and should only be used to visualise discrete numeric variables that have a limited number of possible values. They should never be used to report mean values (see [#barbarplot](https://barbarplots.github.io/) campaign)! 
As we can see from the output of the `unique()` function below, the `Age` variable in `Dabrowska.data` features `r length(unique(Dabrowska.data$Age))` different age values, ranging from `r min(Dabrowska.data$Age)` to `r max(Dabrowska.data$Age)`. 
...
-   [ ] Create and interpret barplots, histograms, density plots, boxplots, scatter plots, and word clouds (@sec-Semantics)
