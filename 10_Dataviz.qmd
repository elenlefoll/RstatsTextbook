...
This chapter is divided into two parts: the first explains the [**syntax**](@Sec-Syntax) of the Grammar of Graphics [@wilkinsonGrammarGraphics2005] and how the {ggplot2} package works, while the second part focuses on the [**semantics**](@sec-Semantics) of statistical graphics and provides an introduction to the many different types of data visualisations that can be created using the package.
...
#### What is alt-text and why is it important? {sec-Alt-text}
...
[**c.**]{style="color:green;"} Create a pie chart that shows the distribution of occupational groups among male participants (as shown in @fig-\*\*). Which line of code is essential to create a pie chart using {ggplot2}?
...
[**d.**]{style="color:green;"} Transform the pie chart that you just created in [Q10.5]{style="color:green;"} to make it look like @fig-\*\*. To achieve this, wrangle the data *before* piping it into the `data` argument of the `ggplot()` function. Which {tidyverse} function can you use to rename the labels?
...
[**a.**]{style="color:green;"} Which line of code needs to be added to the code used to generate @fig-GrammarDensity to produce a two-panel density plot like @fig-\*\*?
...
[**Q10.3**]{style="color:green;"} Create a boxplot to compare how participants in different occupational groups (`OccupGroup`) performed on the English grammar test. Which part of the code used to produce @fig-\*\* <!--# @Vishar this should be the boxplot just above] do you need to modify to achieve this? -->
...
[**Q10.4**]{style="color:green;"} The code below was used to create @fig-\*\*, except that the arguments of the `aes()` function have been deleted. Which data mappings were specified inside the `aes()` function to produce @fig-\*\*?
