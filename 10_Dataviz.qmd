...
Part of the magic of the Grammar of Graphics is that we can easily **combine** **different geometries** and **aesthetics** to create highly informative graphs. If you just read about the `geom_text()` function (see @fig-\*\* <!--# @Vishar, please link to the last plot in the geom_text() callout box above -->), then you have already seen how this can work.

@Fig-\*\* is another example. In this plot, we combine `geom_boxplot()` with `geom_point()` to see both the defining characteristics of the distributions of grammar test scores among L1 and L2 participants, as well as the exact score of each individual participant.
