...
{\textsc{Last updated on}} 16 June 2026
...
\texttt{Ctrl-O} (windows), \texttt{Cmd-O} (mac), \texttt{Ctrl-O} (linux)
...
\texttt{Ctrl-V} (windows), \texttt{Cmd-V} (mac), \texttt{Ctrl-V} (linux)
...
\item
  ...
  \texttt{Ctrl-Shift-N} (windows), \texttt{Cmd-Shift-N} (mac), \texttt{Ctrl-Shift-N} (linux).
\end{enumerate}
...
\texttt{Ctrl-Enter} (windows), \texttt{Cmd-Enter} (mac), \texttt{Ctrl-Enter} (linux).
...
\item
  Use the keyboard shortcut
  \texttt{Ctrl-S} (windows), \texttt{Cmd-S} (mac), \texttt{Ctrl-S} (linux).
\end{enumerate}
...
\texttt{Esc} (windows), \texttt{Esc} (mac), \texttt{Esc} (linux) on your
...
\texttt{Ctrl-Shift-M} (windows), \texttt{Cmd-Shift-M} (mac), \texttt{Ctrl-Shift-M} (linux)
...
\caption{\label{fig-MakingIQR}Screenshot of
\href{https://elenlefoll.github.io/RstatsTextbook/images/quartiles.gif}{animation}
showing the interquartile range of five different sets of values (CC BY
4.0 Elen Le Foll \url{https://doi.org/10.5281/zenodo.17440319})}
...
\caption{\label{fig-MakingBoxplot}Screenshot of
...
keyboard shortcut
\texttt{Ctrl-I} (windows), \texttt{Cmd-I} (mac), \texttt{Ctrl-I} (linux)
...
\item
  ...
  \texttt{Ctrl-Alt-I} (windows), \texttt{Cmd-Option-I} (mac), \texttt{Ctrl-Alt-I} (linux)
\item
...
\item
  ...
  \texttt{Shift-Ctrl-Enter} (windows), \texttt{Shift-Cmd-Enter} (mac), \texttt{Shift-Ctrl-Enter} (linux).
\end{itemize}
...
\texttt{Ctrl-Shift-F8} (windows), \texttt{Cmd-Shift-F8} (mac), \texttt{Ctrl-Shift-F8} (linux).This
...
draft our text messages or type our search query. This is due to four
key factors:
...
\item
  ...
  e.g. Samuelson 2023; Lucchi 2024) scraping of \textbf{huge amounts of
  training data} from the internet, including (academic) books and
  articles, but also vast amounts of blog posts, social media data,
  forum discussions, Wikipedia articles, YouTube videos, etc.
\item
  The availability of (relatively) cheap, large-scale
  \textbf{computational power} (which, however, still comes at a high
  environmental cost, see e.g. Luccioni et al. 2025; Schön, Hoffmann \&
  Becker 2025).
\item
...
LLMs are also randomly generated text strings. They may or may not
correspond to real sources. When the sources exist, they may or may not
contain (some of) the information regurgitated by the LLMs. We have no
way of knowing from the model output.
...
biases (e.g.~the `Matthew effect,' see Pooley 2025). High-quality
research requires us to put in the intellectual effort of searching,
reading, and critically evaluating the literature \emph{ourselves}. Like
writing, this is an integral part of the research process.
...
once we have become dependent, we are no longer able to compare how long
we would have needed to complete the same task had we not relied on a
third-party AI system to do so.
...
appropriate their work and LLMs automate plagiarism (Rooij 2022).

This may sound like an entirely dystopian situation but, up until fairly
recently, researchers had to pay to use programming languages for
statistical analyses and scientific computing. FORTRAN compilers,
MatLab, S, and SPPS were (and still are) proprietary software which were
inaccessible to many researchers and students. It is no exaggeration to
say that open-source, community-led programming languages such as
\texttt{R}, Python, and Julia have revolutionised data analysis, making
state-of-the-art methods accessible to far more people. However, as
high-quality, open-access resources become rarer due the contamination
of AI slop and illegal scraping, we may be returning to an era of
restricted access to scientific computing and educational resources.
This is why it is so important to understand and remember that it is
communities of humans who have been developing programming languages
such as \texttt{R} and Python, their many extensions such as the
\{tidyverse\} packages (see also
...
such as \emph{R Studio}, and high-quality documentation and Open
