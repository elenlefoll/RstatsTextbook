...
several hours of commitment. From Chapter~\ref{sec-Installing} onwards,
all examples come from the same dataset and build on each other. You are
strongly encouraged to try things out on your own computer as you are
working your way through each chapter. In addition, at the end of each
chapter you will find a link to a set of interactive quiz questions
hosted on the textbook's
...
\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-important-color!10!white, colframe=quarto-callout-important-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{What if I absolutely have to open a DSV file in Excel?
\emoji{cold-sweat}}, titlerule=0mm, toprule=.15mm, toptitle=1mm]
...
\section{\texorpdfstring{A word of warning about spreadsheet programs
\emoji{warning}️}{A word of warning about spreadsheet programs ️}}\label{sec-ExcelWarning}
...
\item
  Importing data from LibreOffice/OpenOffice Calc, Microsoft Excel,
...
::: callout-warning \{title=``Case matte\texttt{R}s!''\} Note that,
unlike the functions that we have used so far, the \texttt{View()}
function begins with a capital letter. \texttt{R} is a case-sensitive
programming language, which means that \texttt{view()} and
\texttt{View()} are not the same thing! :::
...
\emoji{woozy-face}}{Hardcoding file paths in R scripts }}\label{sec-HardcodingFilePaths}
...
\subsection{\texorpdfstring{Importing data using RStudio's GUI
\emoji{face-with-rolling-eyes}}{Importing data using RStudio's GUI }}\label{sec-ImportingGUI}
...
observe extreme values.\footnote{If you are not immediately convinced by
  ...
  gets heads all three times. On the basis of these observed data, he
  claims that the coin is biased towards heads. How likely are you to
  believe his claim that the coin is unfair? Probably not very likely.
  But what about if he flips the coin 10 times and it lands on head all
  ...
  0.001\%, which is a one-in-a-thousand occurrence! Of course, this
  doesn't mean that it will \emph{never} happen with a fair coin, but it
  is a very rare occurrence.} This fact is also taken into account by
statistical significance tests such as the \emph{t}-test.
...
\item
  ...
  the default header. Quarto document headers\footnote{Note that, in
    YAML syntax, character strings that include special characters
    (e.g.~\texttt{\textquotesingle{}}) need to be enclosed in quotation
    marks.} are written in \textbf{YAML} which, I kid you not, stands
  for \emph{Yet Another Markup Language}!~\emoji{sweat-smile}
\end{enumerate}
...
A useful starting point to this end is Luis D. Verde Arregoitia's
curated list of \texttt{R} packages and other resources:
...
``\href{https://luisdva.github.io/llmsr-book/before-start.html}{Read
first}'' and
...
p\texttt{R}ojects!
Artwork~\href{https://allisonhorst.com/r-packages-functions}{@allison\_horst}~CC~BY~4.0)}
