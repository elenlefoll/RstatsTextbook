...
\pandocbounded{\includegraphics[keepaspectratio,alt={Sketchnote graphic about Open Educational Resources}]{images/oer.jpg}}
...
Learn more about the Open Practices badges from the Center for Open
Science: \url{https://osf.io/tvyxz/wiki} (Schimke et al. 2018).
\end{quote}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Heading of PDF version of the article from the journal Language Learning: A Journal of Research in Language Studies. EMPIRICAL STUDY First Language Influence on SecondLanguage Offline and Online AmbiguousPronoun Resolution Sarah Schimke, Israel de la Fuente, Barbara Hemforth, and Saveria Colonnada from University Münster, University of Lille/CNRS, CNRS/University of Paris Diderot, and University of Paris 8/CNRS}]{images/SchimkeEA2018Abstract.png}}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={The Open Data badge is blue and shows a simple barplot and the Open Materials is yellow and shows an open cardbox.}]{images/OpenDataOpenMaterials.jpg}}
...
\includegraphics[width=0.6\linewidth,height=\textheight,keepaspectratio,alt={Photo of two Playmobil figures. On the left, a street sweeper with a broom and, on the right, a postman with a trolley.}]{images/Playmobil.png}
...
\includegraphics[width=5.85417in,height=\textheight,keepaspectratio,alt={A screenshot showing a CSV file opened in Microsoft Word. It looks like a long list of words separated by commas.}]{images/DSVinWord.png}
...
\texttt{Ctrl-O} (linux), \texttt{Cmd-O} (mac), \texttt{Ctrl-O} (windows)
...
\includegraphics[width=6.26042in,height=\textheight,keepaspectratio,alt={A screenshot showing a CSV file opened in LibreOffice Calc. It is formatted as a table.}]{images/DSVinCalc.png}
...
\includegraphics[width=0.4\linewidth,height=\textheight,keepaspectratio,alt={A screenshot showing the context menu of a macOS file that can be accessed with a right click. The drop-down option \textquotesingle Get Info\textquotesingle{} is highlighted.}]{images/ChangeDefaultAppMac01.png}
...
\includegraphics[width=0.4\linewidth,height=\textheight,keepaspectratio,alt={A screenshot showing the info dialog of a macOS File Finder window with the \textquotesingle Open with\textquotesingle{} section highlighted. It shows that the Numbers app is the default to open such files on this particular computer.}]{images/ChangeDefaultAppMac02.png}
...
\texttt{Ctrl-V} (linux), \texttt{Cmd-V} (mac), \texttt{Ctrl-V} (windows)
...
\item
  ...
  \texttt{Ctrl-Shift-N} (linux), \texttt{Cmd-Shift-N} (mac), \texttt{Ctrl-Shift-N} (windows).
\end{enumerate}
...
\texttt{Ctrl-Enter} (linux), \texttt{Cmd-Enter} (mac), \texttt{Ctrl-Enter} (windows).
...
\item
  Use the keyboard shortcut
  \texttt{Ctrl-S} (linux), \texttt{Cmd-S} (mac), \texttt{Ctrl-S} (windows).
\end{enumerate}
...
\texttt{Esc} (linux), \texttt{Esc} (mac), \texttt{Esc} (windows) on your
...
\texttt{Ctrl-Shift-M} (linux), \texttt{Cmd-Shift-M} (mac), \texttt{Ctrl-Shift-M} (windows)
...
Note that a slightly more detailed, standalone version of this chapter
is available as
\href{https://elenlefoll.quarto.pub/quarto4research/}{Quarto for
reproducible research workflows and academic publishing: A step-by-step
tutorial} (Le Foll 2026).
...
keyboard shortcut
\texttt{Ctrl-I} (linux), \texttt{Cmd-I} (mac), \texttt{Ctrl-I} (windows)
...
\item
  ...
  \texttt{Ctrl-Alt-I} (linux), \texttt{Cmd-Option-I} (mac), \texttt{Ctrl-Alt-I} (windows)
\item
...
\item
  ...
  \texttt{Shift-Ctrl-Enter} (linux), \texttt{Shift-Cmd-Enter} (mac), \texttt{Shift-Ctrl-Enter} (windows).
\end{itemize}
...
\texttt{Ctrl-Shift-F8} (linux), \texttt{Cmd-Shift-F8} (mac), \texttt{Ctrl-Shift-F8} (windows).This
...
\FunctionTok{cite\_packages}\NormalTok{(}\AttributeTok{output =} \StringTok{"paragraph"}\NormalTok{, }
              ...
              \AttributeTok{pkgs =} \StringTok{"Session"}\NormalTok{,}
              \AttributeTok{omit =} \ConstantTok{NULL}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
2025b), grateful v. 0.3.0 (Rodriguez-Sanchez \& Jackson 2025), gt v.
1.2.0 (Iannone et al. 2025), here v. 1.0.2 (Müller 2025), tidyverse v.
2.0.0 (Wickham et al. 2019), webexercises v. 1.1.0 (Barr \& DeBruine
2023), xfun v. 0.56 (Xie 2026).
...
\section{Rendering formats and options}\label{sec-Publishing}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Dialog menu from Microsoft Word that reads: Word found unreadable content in LearningQuarto3.docx. Do you want to recover the contents of this document? If you trust the source of this document, click Yes. Below the message there are two buttons: Yes and No.}]{images/Word_popup.png}}
...
document into \textbf{presentation slides}. At the time of writing,
there are three presentation formats to choose from:

{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.4356}}
...
interactive dashboards, the world's your oyster! \emoji{rocket}
...
\begin{itemize}
\item
  For those of you who want to dive a little deeper, I heartily
  recommend the final chapter of
...
\item[$\square$]
  ...
  Word, and PDF (\textbf{?@sec-PublishingFormats})
\end{itemize}
...
\section{\texorpdfstring{On the technology behind AI
\emoji{robot}}{On the technology behind AI }}\label{on-the-technology-behind-ai}
...
\includegraphics[width=6.25in,height=\textheight,keepaspectratio,alt={Screen capture showing someone typing "LLMs are not" into Google and various automated completions suggestions including "LLMs are not ai".}]{images/LLMs_fe55f270.png}

}

\caption{\label{fig-autocomplete}Autocomplete function on web search
...
tend to overestimate the quality of LLM outputs in this domain (see also
Tully, Longoni \& Appel 2025). When it comes to learning how to code and
do statistics, political scientist, \texttt{R} package developer, and
educator Andrew Heiss\footnote{I highly recommend Andrew Heiss'
  \href{https://www.andrewheiss.com/blog/}{blog} and his beautiful
...
Assistant) and control (No AI) groups (n~=~52) (Source: Tamkin \& Shen
2026: Fig.~1)}
...
documentation files (e.g.~in the form of so-called
`\href{https://r-pkgs.org/vignettes.html}{vignettes}'). If our web
search did not help us solve our problem, we prepared a \textbf{reprex,}
short for minimal \textbf{repr}oducible \textbf{ex}ample (see Wickham,
Çetinkaya-Rundel \& Grolemund 2023:
\href{https://r4ds.hadley.nz/workflow-help.html}{Ch. 8} on getting
...
{\centering \pandocbounded{\includegraphics[keepaspectratio,alt={Two cute little monsters decorating the R logo with hearts, flowers, and smilies.}]{images/AHorst_RFlowers.png}}

}

\caption{Happy learning and good luck with all your data analysis
p\texttt{R}ojects!️ (artwork by
\href{https://allisonhorst.com/r-packages-functions}{Allison Horst} CC
BY 4.0}
...
\bibitem[\citeproctext]{ref-webexercises}
Barr, Dale \& Lisa DeBruine. 2023. \emph{{webexercises}: Create
interactive web exercises in {``{R Markdown}''} (formerly
{``{webex}''})}.
\url{https://doi.org/10.32614/CRAN.package.webexercises}.
...
Olivier Roy. 2025. \emph{{gt}: Easily create presentation-ready display
tables}. \url{https://doi.org/10.32614/CRAN.package.gt}.
...
academic publishing: {A} step-by-step tutorial. Open \{Educational\}
\{Resource\}. \url{https://doi.org/10.5281/zenodo.18913131}.
