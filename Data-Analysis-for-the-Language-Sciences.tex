...
\protect\phantomsection\label{annotated-cell-351}%
\begin{Shaded}
\begin{Highlighting}[]
...
\protect\phantomsection\label{annotated-cell-388}%
\begin{Shaded}
\begin{Highlighting}[]
...
file and read about the ``.keep'' argument.
...
\protect\phantomsection\label{annotated-cell-452}%
\begin{Shaded}
\begin{Highlighting}[]
...
black, unless otherwise specified with the argument ``colour''.
...
\protect\phantomsection\label{annotated-cell-478}%
\begin{Shaded}
\begin{Highlighting}[]
...
(artwork by \href{https://twitter.com/allison_horst}{Allison Horst} CC
BY 4.0)}
...
\includegraphics[width=4.66667in,height=\textheight,keepaspectratio,alt={Cartoon of a normal distribution looking skeptically at an excited looking bimodal and negatively skewed distribution. The first says to the second, "you\textquotesingle re not normal."}]{images/AHorst_NotNormal.png}

}

\caption{\label{fig-NotNormal}Remember that the best way to check test
assumptions is to visualise your data! (artwork by
\href{https://twitter.com/allison_horst}{Allison Horst} CC BY 4.0)}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Scatter plot of learners\textquotesingle{} test results expressed as the number of correct answers they got on the x-axis and as the percentage of correctly answered questions on the y-axis. The data points form a perfect linear relationship. A straight blue regression line runs through all the data points.}]{12_SimpleLinearRegression_files/figure-pdf/fig-RTScatterPLot-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={This is the same plot as above. The only difference is that a dotted blue line extends the fit blue line to zero.}]{12_SimpleLinearRegression_files/figure-pdf/fig-RTScatterPLotIntercept-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Scatter plot of vocabulary scores on the y-axis, and  years of education on the x-axis. A blue regression line slopes upward, indicating a positive relationship: higher formal education tends to be associated with higher vocabulary scores.}]{12_SimpleLinearRegression_files/figure-pdf/fig-EducationYearsVocab-1.pdf}}
...
\end{verbatim}
  ...
  among L1 speakers.
\item
  The \textbf{adjusted R-squared (R²)} is \texttt{0.1729}, which means
...
\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-note-color!10!white, colframe=quarto-callout-note-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{It's all linear regression!}, titlerule=0mm, toprule=.15mm, toptitle=1mm]
...
Moreover, we said that \textbf{R²} stands for the \textbf{squared
coefficient of correlation}, which is why, if we take the square root of
our unadjusted R² coefficient, we get 0.43, which corresponds to the
...
does the same thing? \emoji{monocle-face} Honestly, we don't.
...
\pandocbounded{\includegraphics[keepaspectratio,alt={A scatter plot showing the relationship between years in formal education and predicted vocabulary scores. The points form an upward linear trend and their size corresponds to the number of people in the L1 dataset who reported being in formal education for that many years. Most people reported spending between 11 and 14 years in formal education but some as many 21 years.}]{12_SimpleLinearRegression_files/figure-pdf/fig-EduPredictedVocab-1.pdf}}
...
\FunctionTok{ggplot}\NormalTok{(L1.data, }
       \FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ pred\_vals, }
           \AttributeTok{y =}\NormalTok{ Vocab)) }\SpecialCharTok{+} 
  \FunctionTok{geom\_point}\NormalTok{() }\SpecialCharTok{+}
  \CommentTok{\# 45° reference line (the "perfect‑prediction" line):}
...
already knew that from the model's fairly low R² value and, crucially,
...
the context of statistical modelling, we can extend the famous saying to
...
\item
  ...
  \pandocbounded{\includegraphics[keepaspectratio,alt={A linear line chart with years as the X-axis and two variables on the Y-axis. The first variable is Air pollution in Johnstown, Pennsylvania and the second variable is Master\textquotesingle s degrees awarded in Education.  The chart goes from 2012 to 2021, and the two variables track closely in value over that time. The following statistics are displayed on the graph: Pearson\textquotesingle s r = 0.989, r-squared = 0.979, p \textless{} 0.01 Source: tylervigen.com/spurious/correlation/3139}]{images/Vigen_unhealthy-air-quality-in-johnstown-pennsylvania_correlates-with_masters-degrees-awarded-in-education.png}}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Box plot comparing non-verbal IQ scores between two groups, L1 and L2. A blue dashed trend line connects the medians, showing that the median L1 score is lower than the median L2 score. Both groups display outliers at the lower end of their distributions.}]{12_SimpleLinearRegression_files/figure-pdf/fig-IQL1L2-1.pdf}}
...
\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-note-color!10!white, colframe=quarto-callout-note-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{It's (still) all linear regression!}, titlerule=0mm, toprule=.15mm, toptitle=1mm]
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Box plot showing predicted vocabulary scores across four occupational groups (C, I, M, and PS). Individual data points are overlaid as dots on each box plot. All groups show substantial variability and some extreme outliers at the lower end of the distribution.}]{12_SimpleLinearRegression_files/figure-pdf/fig-VocabByOccupation-1.pdf}}
...
\item
  ...
  significantly different from 0 at the \(\alpha\)-level of 0.05:
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Box plot showing predicted vocabulary scores across four occupational groups (C, I, M, and PS). Individual residual points are shown as gray dots overlaid on gray box plots with blue median lines.}]{12_SimpleLinearRegression_files/figure-pdf/fig-PredictedVocabByOccupation-1.pdf}}
...
\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-note-color!10!white, colframe=quarto-callout-note-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{What about ANOVAs?}, titlerule=0mm, toprule=.15mm, toptitle=1mm]
...
\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-note-color!10!white, colframe=quarto-callout-note-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Further reading \emoji{books}}, titlerule=0mm, toprule=.15mm, toptitle=1mm]
...
curved pattern and the blue linear regression line is nonsensical. With
these data, making predictions based on this linear model would generate
...
\pandocbounded{\includegraphics[keepaspectratio,alt={A scatter plot comparing word frequency and word rank. The dots follow a curve starting at around word frequency 6,000 and word rank 1, and become increasingly spread out as word frequency decreases and word rank increases, forming a sloping horn shape. A shallow downward-sloping regression line cuts through the lower half of the funnel and ends at around word rank 550.}]{12_SimpleLinearRegression_files/figure-pdf/fig-Nonlinear-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={A scatter plot comparing model residuals and predicted vocab scores. The points fall into a roughly triangular area which becomes more narrow as predicted vocab scores increases. The greatest range of model residuals is found around a predicted vocab score of 67. A perfectly horizontal line cuts through the chart at 0 model residuals.}]{12_SimpleLinearRegression_files/figure-pdf/fig-VocabResidual-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={A density plot of model residuals. The hill rises sharply and peaks just before 0 residuals, marked by a horizontal dotted line, before falling more gradually.}]{12_SimpleLinearRegression_files/figure-pdf/fig-VocabResidualDensity-1.pdf}}
...
regression modelling! \emoji{nerd-face} Coming up in
Chapter~\ref{sec-MLR}\ldots{}
...
predictors using a \emph{z}-transformation (see e.g. Winter 2019:
Section 5.2).
...
\pandocbounded{\includegraphics[keepaspectratio]{images/tbl-centering.png}}
...
\protect\phantomsection\label{annotated-cell-552}%
\begin{Shaded}
\begin{Highlighting}[]
...
\item
  ...
  variance in \texttt{Vocab} scores found in the Dąbrowska (2019) data.
...
displayed. If you have a small screen, you may find the
``\emoji{magnifying-glass-tilted-right} Zoom'' option in RStudio's Plots
pane useful as it allows you to view the plots in a separate window.
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Six-panel scatter plot showing vocabulary scores predicted by various factors, with partial residual points and blue regression lines in each panel. Categorical predictors show relatively flat trends and continuous predictors show positive linear relationships.}]{13_MultipleLinearRegression_files/figure-pdf/fig-VisregVocab-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Scatter plot showing predicted vocabulary scores versus age in years, with partial residual points and a blue regression line with gray confidence band showing a slight positive trend. Two data points are highlighted: a pink point labeled Lithuanian bar staff showing a large positive residual around age 25, and a purple point labeled Polish driver showing a small residual around age 50.}]{13_MultipleLinearRegression_files/figure-pdf/fig-PredictedVocabAge-1.pdf}}
...
e.g. Dąbrowska 2019: 14), lmg has a number of advantages. Its
...
not worry us because a researcher and statistician, Ulrike Grömping, has
developed and published an \texttt{R} package that will do the
computation for us. \emoji{smiling-face-with-smiling-eyes}
...
\end{Shaded}
  ...
  \{dplyr\} package.} (Grömping 2006), we can use its
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Bar plot showing the relative importance of six predictors in model4\_c, with predictor names on the y-axis and lmg values on the x-axis. Group has the highest importance and Gender has the lowest one.}]{13_MultipleLinearRegression_files/figure-pdf/fig-ModelLmg-1.pdf}}
...
Winter 2019: Section 6.2); however, this approach can lead to misleading
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Two-panel scatter plot showing vocabulary scores versus age for L1 and L2 groups, with data points and blue regression lines with gray confidence bands. The L1 panel shows a positive correlation, while the L2 panel shows a negative correlation.}]{13_MultipleLinearRegression_files/figure-pdf/fig-CorVocabAge-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Two-panel plot showing vocabulary scores versus age for L1 and L2 groups, with data points and blue regression lines with gray confidence bands. The L1 panel shows a positive correlation, while the L2 panel shows a negative correlation.}]{13_MultipleLinearRegression_files/figure-pdf/fig-PredictedVocabAgeInteraction-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Visreg plot showing predicted vocabulary scores versus age for three education levels, with color-coded partial residual points and regression lines with confidence bands. The red line is rising from approximately 55 to 80, the green line is rising from about 65 to 90, and the blue lines is rising from approximately 75 to 95.}]{13_MultipleLinearRegression_files/figure-pdf/fig-PredictedVocabAgeRL-1.pdf}}
...
\protect\phantomsection\label{annotated-cell-289}%
\begin{Shaded}
\begin{Highlighting}[]
...
\protect\phantomsection\label{annotated-cell-290}%
\begin{Shaded}
\begin{Highlighting}[]
\FloatTok{51.9162} \SpecialCharTok{+} \hspace*{\fill}\NormalTok{\circled{1}}
\FloatTok{1.3088} \SpecialCharTok{*} \DecValTok{31} \SpecialCharTok{+} \hspace*{\fill}\NormalTok{\circled{2}}
...
significant at \(\alpha\)~=~0.05. The main effects of these two
predictors cannot be meaningfully interpreted without considering this
interaction.
...
the accuracy in predicting future outcomes. (Çetinkaya-Rundel \& Hardin
2026:
...
model's coefficient estimates (sometimes referred to as β, ``beta''),
...
(Lüdecke 2020) includes a handy function that produces nicely formatted
...
ART × EduTotal c & -0.15 & -0.24~--~-0.05 & \textbf{0.002} \\
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Scatter plot showing predicted vocabulary scores versus years in formal education for L1 speakers, with partial residual points and a blue regression line with gray confidence band. The plot shows a positive correlation, with predicted scores increasing from approximately 57 at 10 years of education to approximately 87 at 21 years of education.}]{13_MultipleLinearRegression_files/figure-pdf/fig-PredictedVocabEduL1-1.pdf}}
...
\CommentTok{\#| fig{-}alt: "A density chart comparing observed vocabulary scores with scores predicted by the model. A green line which plateaus slightly from about 50 to 70 at densities from 0.013 to 0.015 and peaks at around 80 with a density of 0.020 is overlapped by several transparent blue lines, all of which peak at lower test scores."}
\end{Highlighting}
\end{Shaded}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={A scatter plot relating model predictions and model residuals. The majority of the dots fall between -10 and 20 residuals, and a very slightly curved line cuts through them. It begins slightly below zero, peaks slightly above zero at about 80 fitted values, and falls back below zero. The linearity is indicated by a large shaded region surrounding the line.}]{13_MultipleLinearRegression_files/figure-pdf/fig-Linearity-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Another scatter plot indicating the linearity of the relationship between fitted values and the square root of absolute standardised residuals. Most of the dots fall between 0.3 and 1.3 standardised residuals. The linearity line begins relatively level at around 0.9 standard residuals and dips to around 0.6 between fitted values 70 and 80.}]{13_MultipleLinearRegression_files/figure-pdf/fig-Homoscedasticity-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={A chart providing the variance inflation factor and 95\% confidence interval for each of the model predictors: age c, ART, ART:edutotal c, blocks c, and edutotal c. age c, art, and blocks c have lower variance inflation factors and smaller confidence intervals than art:edutotal c and edutotal c.}]{13_MultipleLinearRegression_files/figure-pdf/fig-Collinearity-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={A scatter plot relating standard normal distribution quantities and sample quantile deviations. Most of the dots are clustered around the line at 0 sample quantile deviations from standard normal distribution quantities -1 to 2, outside of which they are more scattered. All of the dots are encapsulated by a shaded area which thins at the middle.}]{13_MultipleLinearRegression_files/figure-pdf/fig-Normality-1.pdf}}
...
\item
  ...
  \item
    ...
    Sonderegger 2023: Chapter~6; Winter 2019: Chapter~12).
  \item
  ...
  \item
    ...
    find out more, see e.g. Winter 2019: Chapter~13).
  \end{itemize}
\item
  ...
  multilevel or hierarchical models) (to learn more, see e.g. Vasishth
  et al. 2022; Sonderegger 2023: Chapters~8-10; Winter 2019:
  Chapters~14-15).
\item
...
and quizzes} will help you consolidate all this new knowledge. Good
luck! \emoji{four-leaf-clover}
...
Note that a standalone version of this chapter can be found at
...
\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-note-color!10!white, colframe=quarto-callout-note-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Going further️ \emoji{rocket}}, titlerule=0mm, toprule=.15mm, toptitle=1mm]
...
\pandocbounded{\includegraphics[keepaspectratio,alt={A screenshot of the new document settings in Quarto. The format selected is HTML as opposed to PDF or Word Document, no engine is selected, and visual markdown editor is not enabled.}]{images/Quarto_new.png}}
...
Markdown is gaining in popularity and is now widely supported across
many platforms, from text editors to content management systems,
ensuring that your formatting remains consistent and portable. It's
increasingly used to write documentation, blog posts, or simply to take
notes. Check out the
...
Insert the following \texttt{R} chunk to load the Dąbrowska (2019) data
so that they may be used in your Quarto document. As this
...
\begin{quote}
``Ninety native speakers (42 male and 48 female) and 67 nonnative
speakers of English (21 male and 46 female) were recruited through
personal contacts, church and social clubs, and advertisements in local
...
\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-note-color!10!white, colframe=quarto-callout-note-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Counting words}, titlerule=0mm, toprule=.15mm, toptitle=1mm]
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{::: \{\#fig{-}Pipelines layout{-}ncol="2"\}\{\#fig{-}alt: "The earlier graphic with 3 entry points for data."\}}
\AlertTok{![An idealised research pipeline](images/BERD\_pipeline{-}simple.jpg)}\NormalTok{\{\#fig{-}IdealisedPipeline\}}

\AlertTok{![A more realistic research pipeline](images/BERD\_pipeline{-}real.jpg)}\NormalTok{\{\#fig{-}RealisticPipeline2\}\{\#fig{-}alt: "The earlier, more realistic pipeline graphic."\}}
...
\CommentTok{\#| fig{-}alt: "A scatter plot relating vocabulary and collocation test scores. The dots are coloured according to occupational group. A slightly upward sloping regression line cuts through the dots, indicating a positive correlation."}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={A scatter plot relating vocabulary and collocation test scores. The dots are coloured according to occupational group. A slightly upward sloping regression line cuts through the dots, indicating a positive correlation.}]{14_LiterateProgramming_files/figure-pdf/fig-scatterplot-1.pdf}}
...
scaling in R is ``an art and science and getting things right can
require an iterative trial-and-error approach'' (Wickham,
Çetinkaya-Rundel \& Grolemund 2023). This is because there are five main
options that control figure sizing: \texttt{fig-width},
...
\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-note-color!10!white, colframe=quarto-callout-note-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Literature management \emoji{books}}, titlerule=0mm, toprule=.15mm, toptitle=1mm]
...
Combining Zotero and Quarto also allows you to generate annotated
bibliographies. Benjamin Tjepkes explains how to do this in a detailed
...
first installed the package, load the \{grateful\} library:
...
Make sure that all the packages that your script relies on are loaded
and then run the following command once to generate a bibliography of
all loaded packages:
...
checkdown v. 0.0.13 (Moroz 2020), grateful v. 0.3.0 (Rodriguez-Sanchez
\& Jackson 2025), gt v. 1.2.0 (Iannone et al. 2025), here v. 1.0.2
(Müller 2025), tidyverse v. 2.0.0 (Wickham et al. 2019), webexercises v.
1.1.0 (Barr \& DeBruine 2023), xfun v. 0.55 (Xie 2025).
...
lists the packages used in this chapter. To display functioning links
and references, the table is rendered using the \texttt{kable()}
...
the scope of this tutorial, learning Git has the potential to greatly
...
by default, Quarto keeps external resources separate from the main HTML
file. While this is advantageous for large documents and complex
...
file with someone else, you will need to \textbf{embed} all the
necessary files directly inside your HTML file. This is achieved by
adding the following option at the end of your document's YAML header:
...
\includegraphics[width=4.86458in,height=\textheight,keepaspectratio,alt={A schematic showing many qmd files, going through Quarto, to generate an HTML, PDF, or Word document or more. The Quarto logo is depicted as a baseball a penguin is spinning.}]{images/AHorst_many-qmd-to-output.png}
...
Quarto'' keynote} by Julia Lowndes and Mine Çetinkaya-Rundel, presented
...
\includegraphics[width=2.46875in,height=\textheight,keepaspectratio,alt={Dialog menu from Microsoft Word that reads: Word found unreadable content in LearningQuarto3.docx. Do you want to recover the contents of this document? If you trust the source of this document, click Yes. Below the message there are two buttons: Yes and No.}]{images/Word_popup.png}
...
Gries, Stefan Th. 2021. \emph{Statistics for linguistics with {R}: A
practical introduction} (Mouton {T}extbook). 3. edn. Berlin/Boston: de
Gruyter Mouton. \url{https://doi.org/10.1515/9783110718256}.
...
Lüdecke, Daniel. 2020. \emph{sjPlot: Data visualization for statistics
...
4(43). \url{https://doi.org/10.21105/joss.01686}.
...
Xie, Yihui. 2025. \emph{{xfun}: Supporting functions for packages
