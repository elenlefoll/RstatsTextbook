...
When there are many different unique values (or **levels**), it makes sense to order them according to their frequency. To do so, we can pipe the output of the `table()` function into the `sort()` function. Note that, by default, `R` sorts by ascending order (`decreasing = FALSE`). We can change this default to `TRUE`.
...
```{r}
#| code-fold: true
#| code-summary: "Show the code to produce the plot below (note that we will learn how to create bar charts and other types of plots in Chapter 10)."
#| fig-width: 8
#| fig-height: 5
