...
Means are also frequently misinterpreted as "most likely value". This is rarely the case. In fact, in the example above, `r mean(c(89, 91, 86, 5, 82))` is not even a score that any of the five students obtained!
...
check_question(c("66.31944", "66.31", "66.32", "66", "66.3", "66,31944", "66,31", "66,32", "66,3"),
...
check_question(c("29.47761", "29.47", "29.48", "29", "29.478", "29,47761", "29,47", "29,48", "29,478"),
button_label = "Check answer",
q_id = "Q8.2", 
right = "Perfect!",
...
check_question(c("68.75", "68.7", "68.8", "69", "68,75", "68,7", "68,8"),
button_label = "Check answer",
q_id = "Q8.3", 
right = "That's right, well done!",
...
check_question(c("18.75", "18.7", "18.8", "19", "18,75", "18,7", "18,8"),
button_label = "Check answer",
q_id = "Q8.4", 
...
[^8_descriptivestats-3]: A Likert scale is a type of rating scale used to measure attitudes, opinions, or feelings. It typically consists of a series of statements or questions with a range of possible responses, often on a scale from "strongly disagree" to "strongly agree". For example, in a study on language attitudes, participants might be asked to rate their agreement with the statement "I think it's important to speak standard English in formal situations" on a scale from "1 (strongly disagree)" to "5 (strongly agree)". The resulting variable will therefore consist of numbers ranging between 1 and 5. Although they are often incorrectly treated as such, Likert scales are not numeric variables, but rather ordinal variables (see @sec-Variables). This is because the numbers refer to different categories that describe an order of responses, rather than an actual measured quantity.
...
When computing proportions, `0` corresponds to 0% and `1` to 100%. If we want to obtain percentages, we multiply these numbers by 100. Here, we can see that more than 90% of English native speakers in @DabrowskaExperienceAptitudeIndividual2019 reported not being competent in any language other than English.
...
L1.data |> 
  ...
  ggplot(aes(x = fct_infreq(OtherLgs), 
             y =..count../sum(..count..))) +
  geom_bar() +
  facet_wrap(~ Gender) +
  theme_bw() +
...
In @sec-DataViz, you will learn how to make plots like @fig-OtherLgsGender in `R` using the {ggplot2} package.
...
As the above table contains a lot of different values, it's easier to visualise these numbers in the form of a **bar chart** (also called **barplot**) (see @fig-BarChartAgeMode). The mode (`28`) has been highlighted in black.

```{r}
#| label: fig-BarChartAgeMode
#| code-fold: true
#| echo: !expr "knitr::is_html_output()"
#| code-summary: "Show the `R` code to produce the plot below."
#| fig-width: 8
#| fig-height: 5
#| fig-cap: "Bar chart of the age distribution of L2 participants in @DabrowskaExperienceAptitudeIndividual2019."
#| fig-alt: "Barplot showing the distribution of L2 participants’ ages. The y-axis represents the number of L2 participants and the x-axis represents their age. A single bar corresponding to 28 years of age on the x-axis is highlighted as the mode for having the highest number of participants. The next three highest bars correspond to 22, 31, and 39 years of age."

barplot.mode <- 
  L2.data |> # <1>
  ggplot(mapping = aes(x = Age)) + # <2>
  geom_bar(aes(fill = (Age == 28))) + # <3> 
  scale_fill_manual(values = c("TRUE" = "black"), guide = "none") + # <4> 
  theme_classic() + # <5>
  scale_y_continuous(name = "Number of L2 participants", # <6>
                     breaks = scales::breaks_width(1), 
                     expand = c(0, 0)) + 
  scale_x_continuous(breaks = scales::breaks_width(1), # <7>
                     expand = c(0, 0)) + 
  annotate(geom = "text", # <8>
           x = 25, 
           y = 5.8, 
           label = "mode", 
           colour = "black",
           family = "mono") + 
  annotate(geom = "curve", # <9>
    x = 25,
    y = 5.65, 
    xend = 27.2,
    yend = 5, 
...
1.  We the L2.data data frame and pipe it into the `ggplot()` function.
2.  We start a ggplot object, mapping `Age` to the *x*-axis.
3.  We add a barplot layer, conditionally fill the bars: bars representing 28 years of age will have a different colour.
4.  We manually control the colours of the bar fill, setting the bar representing `Age == 28` to "#0000EE" and remove the legend.
5.  We apply ggplot2's classic theme.
6.  We ensure that there are tick marks for every single whole number and do not extend the limits of *y*-scale to avoid white space on the plot.
7.  We set the *x*-axis breaks and remove white space.
8.  We add label for the mode.
9.  We add a curved arrow for the mode label.
10. We print the plot that we saved to the local environment under this name.
:::

::: {.content-visible when-format="pdf"}
As the above table contains a lot of different values, it's easier to visualise these numbers in the form of a **bar chart** (also called **barplot**) (see @fig-BarChartAgeModePDF). The mode (`28`) has been highlighted in black.

```{r}
#| label: fig-BarChartAgeModePDF
#| echo: false
#| fig-width: 8
#| fig-height: 5
#| fig-cap: "Bar chart of the age distribution of L2 participants in @DabrowskaExperienceAptitudeIndividual2019."
...
Inspecting the full distribution of ages in a plot, it's much easier to see that the second most frequent ages after the mode of `28` are `22`, `31` and `39`. Now turn to @fig-BarChartAgeMedian. How do these ages compare to the median age?
...
#| fig-width: 8
#| fig-height: 5
#| label: fig-BarChartAgeMedian
#| fig-cap: "Bar chart of the age distribution of L2 participants in @DabrowskaExperienceAptitudeIndividual2019."
#| fig-alt: "This is the same barplot as above. The only difference is that a single bar is highlighted in red as the median: it corresponds to 31 years."
...
#| fig-height: 5
#| fig-cap: "Bar chart of the age distribution of L2 participants in @DabrowskaExperienceAptitudeIndividual2019"
#| fig-alt: "This is the same barplot as above. The mean is marked by a blue dashed line and it corresponds to 32.72 years on the x-axis."
...
Next, we can reduce the number of bars by adding together the number of L2 participants aged `20`-`22`, `22`-`24`, `24`-`26`, etc. This is what we call a **histogram** (see @fig-HistogramAges). Histograms are used to visualise distributions and their bars are called **bins** because they "bin together" a number of values.

```{r}
#| label: fig-HistogramAges
#| code-fold: true
#| code-summary: "Show `R` code to generate the plot below."
#| echo: !expr "knitr::is_html_output()"
#| fig-width: 8
#| fig-height: 5
#| fig-cap: "Histogram of the age distribution of L2 participants in @DabrowskaExperienceAptitudeIndividual2019"
...
If we reduce the number of bins by having them cover three years instead of two, the shape of the histogram is simplified (see @fig-HistogramAges3Y).

```{r}
#| code-fold: true
#| label: fig-HistogramAges3Y
#| code-summary: "Show `R` code to generate the plot below."
#| echo: !expr "knitr::is_html_output()"
#| fig-width: 8
#| fig-height: 5
#| fig-cap: "Histogram of the age distribution of L2 participants with a binwidth of three years"
#| fig-alt: "This is the same histogram as above. The only difference is that the bins have ranges of three years rather of two, which conveys a less granular distribution of age values, only giving the reader the broad impression of how the values are distributed."
...
[^8_descriptivestats-4]: Of course, there is an `R` function to help you do the maths! The `ecdf()` function allows us to calculate the area under the curve between the ages of 42 and 62.
...
Perfectly normal distributions, however, are very rarely found in real life! @fig-NormalDistribution shows what the normal distribution of 1,000 participants' age might look like in real life (using numbers randomly generated from a perfectly normal distribution thanks to the `R` function `rnorm()`).
...
#| fig-cap: "A normal distribution showing the age of a fictitious group of 1,000 participants between 10 and 100 years old with a mean of 35 and a standard deviation of 8 (see @sec-SD)"
...
set.seed(42)
normal.age.sd8 <- round(rtruncnorm(mean = 35, sd = 8, n = 1000, a = 10, b = 100))
...
check_question("The difference between the mean and median age is greater in the L1 group than in the L2 group.",
               ...
               button_label = "Check answer",
               right = "Correct!",
               wrong = "No, that's incorrect. Only one statement is correct.")
```
...
#| fig-cap: "Density plot showing the distribution of L1 participants' ages in @DabrowskaExperienceAptitudeIndividual2019"
#| fig-width: 8
#| fig-height: 5
...
wrong = "No, not quite.")
check_hint("Two of the above statements are assumptions that we can make based on the available data.", hint_title = "🐭 Click on the mouse for a hint.")
...
random_answer_order = TRUE,
button_label = "Check answer",
right = "Great stuff, well done! Now, it's time to move on to measures of variability. 🤓",
wrong = "No, not quite.")
...
Understanding and being able to recognise the characteristics of a normal distribution is important as many statistical tests assume that the variables entered in these tests are (approximately) normally distributed (see @sec-Inferential and @fig-NotNormal).
...
#| fig-cap: "Three simulated normal distributions of ages, all with a mean/median of 35 years."
#| fig-alt: "Three barplots showing the ages of three different groups of people. They all resemble normal distributions with means and medians around 35 years but differ in variability with barplot A including values from 32 to 38, barplot B including values from 18 to 50 and barplot C including values from 10 to 66 years."
...
The most basic measure of variability is range. It is calculated by subtracting the highest value of a variable from its lowest value. For example, in @DabrowskaExperienceAptitudeIndividual2019, the range of results obtained by the L1 participants in the English grammar comprehension test is:
...
In practice, the range is usually reported by explicitly mentioning a variable's lowest and highest values as this is usually much more informative than the range itself. Here is how @DabrowskaExperienceAptitudeIndividual2019 reports the age of the participants when describing her data:
...
-   The **second quartile (Q~2~)** is the **median**. Half of the data (25% + 25% = 50%) are below this value, the other half are above.
...
::: {.content-visible when-format="html"}
![Animation showing the interquartile range of five different sets of values (CC BY 4.0 Elen Le Foll <https://doi.org/10.5281/zenodo.17440319>)](images/quartiles.gif){#fig-MakingIQR fig-alt="Animation showing the interquartile range of different sets of values. The set of values are listed in one column and aligned at their respective medians. The first and third quartiles are highlighted and the range between them is labeled interquartile range."}
:::

::: {.content-visible when-format="pdf"}
![Screenshot of animation showing the interquartile range of five different sets of values (CC BY 4.0 Elen Le Foll <https://doi.org/10.5281/zenodo.17440319>)](images/GifCh8a.png){#fig-MakingIQR fig-alt="Still of an animation showing the interquartile range of different sets of values. The set of values are listed in one column and aligned at their respective medians. The first and third quartiles are highlighted and the range between them is labeled interquartile range."}
:::
...
[^8_descriptivestats-5]: Quartiles can also be computed using the `quantile()` function, which takes two arguments: the variable and a value between 0 and 1 corresponding to our **quantile** of interest. We are interested in the **first** and **third quartiles**, therefore in the values below which lie one quarter (`0.25`) and three-quarters (`0.75`) of all the data.
    ...
    To compute the first (Q~1~) and third quantile (Q~3~), we therefore enter:
...
::: {.content-visible when-format="html"}
![Animation showing the making of a boxplot (CC BY 4.0 Elen Le Foll <https://doi.org/10.5281/zenodo.17440384>)](images/boxplot.gif){#fig-MakingBoxplot fig-alt="Animation showing the making of a boxplot. The minimum, the maximum, the median and the other quartiles of a set of values get highlighted. As the set is turned into a boxplot, a box depicts the interquartile range with two whiskers connecting the sides of the box to the maximum and minimum. The median is also highlighted."}
:::

::: {.content-visible when-format="pdf"}
![Screenshots of [animation](https://elenlefoll.github.io/RstatsTextbook/images/boxplot.gif) showing the making of a boxplot (CC BY 4.0 Elen Le Foll <https://doi.org/10.5281/zenodo.17440384>)](images/GifCh8b.png){#fig-MakingBoxplot fig-alt="Three stills of an animation showing the making of a boxplot. The minimum, the maximum, the median and the other quartiles of a set of values get highlighted. As the set is turned into a boxplot, a box depicts the interquartile range with two whiskers connecting the sides of the box to the maximum and minimum. The median is also highlighted."}
:::
...
#| fig-cap: "Symmetrical boxplots showing three different normal distributions"
...
[**Q8.16**]{style="color:green;"} The boxplots below are based on the same data as the three density plots in @fig-3NormalDistributions. Compare the two figures. Which distribution corresponds to which boxplot?
...
Are the scores visualised in the plot above normally distributed?
...
Based on the outputs of the `summary()` function, what does the plot above display?
...
And the good news is that there is a base `R` function to calculate the standard deviation. It is called `sd()`. However, if we use the `sd()` function to calculate the standard deviation of our five Breton learners' test scores, we get a slightly different result... `r emoji("monocle_face")`

```{r}
sd(Breton.scores)
```

This is because, in practice, we almost always divide the sum of squares not by the total number of data points (*N* ), but by the total number minus one (*N*-1). This is the difference between the **population standard deviation** and the **sample standard deviation**. The population standard deviation is used when we have access to the entire population (e.g. all L2 English users living in the UK!), which is exceedingly rare in real-world research. In most cases, we work with samples [e.g. as in @DabrowskaExperienceAptitudeIndividual2019, a sample of 67 L2 English users]. Dividing by *N*-1 gives us a more accurate estimate of the population's standard deviation based on our sample. It helps to reduce the bias in our estimate, making it a more reliable measure of variability around the mean.
...
With a normal distribution, the standard deviation informs us about the width of the bell around the central tendency. @fig-3NormalDistributions shows three normal distributions, all with a median/mean of 35, but with different bell shapes. This is because they have very different standard deviations around that central tendency of 35 years. Let us compare the distribution shapes of these three distributions:

::: {.content-visible when-format="html"}
Distribution A (@fig-DensityA) is a normal distribution with a mean of 35 years (M = 35) and a standard deviation of one year (*SD* = 1).
...
#| fig-alt: "A barplot of an age distribution overlaid with a density curve. The curve is symmetrical and bell-shaped with a median highlighted at 35 years and values from 32 to 38 years."

normal.density(normal.age.A)
```

Distribution B (@fig-DensityB) is a normal distribution with a mean of 35 years (M = 35) and a standard deviation of 5 years (*SD* = 5).
...
#| fig-alt: "A barplot of an age distribution overlaid with a density curve. The bell curve is a lot wider than in the plot above: it features values from 18 to 52 years."

normal.density(normal.age.B) +
  labs(title = "B")
```

Distribution C (@fig-DensityC) is a normal distribution with a mean of 35 years (M = 35) and a standard deviation of 10 years (*SD* = 10).
...
#| fig-alt: "A barplot of an age distribution overlaid with a density curve. The bell curve is even wider than in the last two plots: it features values from 10 to 66 years."
...
::: {.content-visible when-format="pdf"}
-   Distribution A in @fig-3NormalDistributions is a normal distribution with a mean of 35 years (M = 35) and a standard deviation of one year (*SD* = 1).
...
::: {.content-visible when-format="html"}
In this respect, boxplots are more informative (compare the above standard deviations with @fig-GrammarRBoxplots). To evaluate the full shape of a numeric variable's distribution, however, it is best to plot the full distribution as a histogram or density plot.
:::
...
::: callout-note
#### Developping a visual understanding of standard deviation

As a follow-up, I highly recommend reading this short and highly accessible [article](https://medium.com/data-science/a-visual-interpretation-of-the-standard-deviation-30f4676c291c) by Fahd @alhazmiVisualInterpretationStandard2020, who provides a beautifully intuitive guide to understanding standard deviation with graphical animations.
:::
...
It's time to complete this chapter's [tasks and quizzes](https://elenlefoll.github.io/RstatsTextbook/quizzes/8_quiz.html). They will give you the opportunity to continue to explore the datasets from @DabrowskaExperienceAptitudeIndividual2019, ensuring that you are ready to tackle data wrangling, visualisation, and inferential statistics in `R` in the up-coming chapters!
:::
...
In @sec-DataViz, we will cover the basics of **data visualisation** and learn how to create a range of informative and elegant plots (including histograms and density plots) using the popular `R` package `ggplot2`. But, first, we need to learn about **data wrangling** (@sec-DataWrangling) to prepare our data for data visualisation and multivariable analyses. Are you ready? `r emoji("nerd_face")`
