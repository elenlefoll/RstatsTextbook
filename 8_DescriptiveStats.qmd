...
-   Calculate the mode, mean, median of a numeric variable in `R`
...
-   Interpret the distribution of a numeric variable summarised as a boxplot
-   Interpret and calculate the standard deviation
...
This average grade does not describe very well how *any* of the students did: Four did much better than that, while one did considerably worse! The results of quantitative studies, however, typically involve much larger datasets so that averages *can* be a very useful way to describe central tendencies within the data. But it's important to understand that, depending on the data, different measures of central tendency make sense. Later on, we will also see that measures of central tendency do not suffice to describe numeric variables: **measures of variability** (@sec-Variability) and good **data visualisation** (Chapter 10) are also crucial.

### Mean {#sec-Mean}

The measure of central tendency that we have looked at so far is the mean. When people speak of averages, they typically *mean* mean values.
...
From this output, we can tell that the mode of the `Gender` variable in the L1 dataset is `F` for "female".

When there are many different unique values (or **levels**), it makes sense to order them according to their frequency. To do so, we can pipe the output of the `table()` function into the `sort()` function. Note that, by default, `R` sorts by ascending order (`decreasing = FALSE`), but we can change this default to `TRUE`.
...
[**Q1.**]{style="color:green;"} What was the mean age of the L1 participants in @DabrowskaExperienceAptitudeIndividual2019? Use the `round()` function to round off the mean value to two decimal places (see @sec-Piping for a reminder as to how to combine two functions).
...
[**Q2.**]{style="color:green;"} On average, were the L1 participants in @DabrowskaExperienceAptitudeIndividual2019 older or younger than the L2 participants?
...
[**Q3.**]{style="color:green;"} Which of the following statements is true about the L1 and L2 participants in @DabrowskaExperienceAptitudeIndividual2019?
...
[**Q4.**]{style="color:green;"} What were the modal ages of the L1 and L2 participants in @DabrowskaExperienceAptitudeIndividual2019?
...
To round the values to two decimal places, we can apply the `round()` function to the `R` object that we created in the previous chunk (`OtherLgs.prop`).

```{r}
round(OtherLgs.prop, digits = 2)
```
...
In Chapter 10, you will learn how to make plots like @fig-OtherLgsGender in `R` using the {ggplot2} package.
...
As the above table contains a lot of different values, it's easier to visualise these numbers in the form of a **bar chart**. The mode (`28`) has been highlighted in black.

```{r}
#| code-fold: true
#| code-summary: "Show the code to produce the plot below (note that you will learn how to create bar charts and other types of plots in Chapter 11)."
#| fig-width: 8
#| fig-height: 5
...
We can compare the mode (`18`) and median (`31`) to the mean (`32.72`), which, on the following bar chart, is represented as a blue dashed line.
...
If we wanted to work out the probability of an L2 participant being between `42` and `62` years old, we would have to calculate the **area under the curve** between these two points on the *x*-axis. Even without doing any maths, you can see that this area is considerably smaller than between the ages of `22` and `42`. This means that, in this dataset, participants are considerably more likely to be between 22 and 42 than between 42 and 62 years old.[^8_univariatedescriptivestats_inprogress-2]

[^8_univariatedescriptivestats_inprogress-2]: Of course, there is an `R` function to help you do the maths! The `edcf()` function allows us to calculate the area under the curve between the ages of 42 and 62.
...
In a perfectly normally distributed variable, the mean and the median are exactly the same. They are both found at the centre of the distribution and the bell shape of the distribution is perfectly symmetrical. Perfectly normal distributions, however, are very rarely found in real life! Here is what the **normal distribution** of 1,000 participants' age might look like in real life (using numbers randomly generated from a perfectly normal distribution thanks to the `R` function `rnorm()`).
...
In @fig-NormalDistribution, the mean (`r mean(normal.age.sd8)`) and the median (`r median(normal.age.sd8)`) are very close to each other. So close that, on the plot, the lines overlap. The density curve is an almost symmetrical bell shape. This means that the purple area under the curve to the left of the central tendency is pretty much the same as the area to the right of the line. In other words, there are as many people whose age is below the central tendency (50%) as there are people whose age is above the central tendency (50%). These are the characteristics of a normal distribution. Many statistical tests assume that variables are (approximately) normally distributed.
...
[**Q5.**]{style="color:green;"} What is the mode of L1 participants' age?

```{r echo=FALSE, results="asis"}
check_question("17",
button_label = "Check answer",
right = "That's correct! Note, also, that the second most frequent age is 60.",
wrong = "No, that's incorrect. Careful examine the output of the `table(L1.data$Age` command.`")
check_hint("Use the `table()` function to find out what the most frequent age is in the L1 dataset.", hint_title = "🐭 Click on the mouse for a hint.")

```

 

[**Q6.**]{style="color:green;"} Which of the following statements is true?
...
[**Q7.**]{style="color:green;"} Which measure of central tendency best describes L1 participants' ages?
...
[**Q8.**]{style="color:green;"} What can we reasonably deduce from the distribution of L1 participants' ages visualised in @fig-L1AgeDensityPlot?
...
[**Q9.**]{style="color:green;"} What are the pros of reporting the median rather than the mean to describe the central tendency of a variable?
...
[**Q10.**]{style="color:green;"} What are the cons of reporting the median rather than the mean to describe the central tendency of a variable?
...
> On average, English native speakers performed only marginally better in the English grammatical comprehension test (mean = `r median(L1.data$GrammarR)`) than English L2 learners (mean = `r median(L2.data$GrammarR)`). L1 participants' grammatical comprehension test results ranged from `r min(L1.data$GrammarR)` to `r max(L1.data$GrammarR)`. In this same test, L2 participants' results ranged `r min(L2.data$GrammarR)` to `r max(L2.data$GrammarR)`.
...
We can further subdivide the distribution into chunks of 25% of the data, or **quartiles**.
...
![](images/quartiles.gif)

The easiest way to examine a variable's IQR in `R` is to use the handy `summary()` function which, when applied to a numeric variable, returns a number of useful descriptive statistics including its first and third quartiles.[^8_univariatedescriptivestats_inprogress-3]

[^8_univariatedescriptivestats_inprogress-3]: Quartiles can also be computed using the `quantile()` function, which takes two arguments: the variable and a value between 0 and 1 corresponding to our **quantile** of interest. We are interested in the **first** and **third quartiles**, therefore in the values below which lie one quarter (`0.25`) and three-quarters (`0.75`) of all the data.
...
[**Q11.**]{style="color:green;"} Examine the boxplots displayed in @fig-NormalBoxplots.
...
check_question(c("Ages in Group 1 are normally distributed.",
               ...
               right = "That's right!",
               wrong = "Incorrect.")
check_hint("Remember that normal distributions are characterised by a symmetrical distribution of data points around the median.", hint_title = "🐭 Click on the mouse for a hint.")
```

 

[**Q12.**]{style="color:green;"} The boxplots in @fig-NormalBoxplots are based on the same data as the three density plots in @fig-3NormalDistributions. Compare the two figures. Which distribution corresponds to which boxplot?
...
[**Q13.**]{style="color:green;"} Examine the following distribution of scores on the grammatical comprehension test administered as part of @DabrowskaExperienceAptitudeIndividual2019.
...
check_question("No, they are far from normally distributed.",
               options = c("No, they are far from normally distributed.",
                           ...
                           "They are approximately normally distributed but with a slight positive skew."),
               type = "radio",
               random_answer_order = TRUE,
               button_label = "Check answer",
               right = "Correct!",
...
[**Q14.**]{style="color:green;"} Compare the following outputs of the `summary()` function.
...
[**Q15.**]{style="color:green;"} Compare the following boxplots which summarise the distribution of scores on the grammatical comprehension test (`GrammarR`) administered as part of @DabrowskaExperienceAptitudeIndividual2019.
...
ggplot(mapping = aes(y = Scores, x = Group), 
...
This is the **variance**. The problem with the variance is that it is not in the original scale of our variable, but rather in squared units, i.e., here, in squared test scores, which is rather difficult to interpret! This is why we more commonly report the **standard deviation**, which is the square root of the variance (`sqrt()`).

```{r}
sqrt(sum((Breton.scores - mean(Breton.scores))^2) / 5)
```

This means that, on average, learners' test scores are 32 points away from the group mean of `r mean(Breton.scores)` points.
...
Distribution A is a normal distribution with a mean of 35 years (x̅ = 35) and a standard deviation of one year (sd = 1).
...
Distribution B is a normal distribution with a mean of 35 years (x̅ = 35) and a standard deviation of 5 years (sd = 5).
...
Distribution C is a normal distribution with a mean of 35 years (x̅ = 35) and a standard deviation of 10 years (sd = 1).
...
[**Q16.**]{style="color:green;"} What is the standard deviation of L1 participants' age in @DabrowskaExperienceAptitudeIndividual2019? Calculate the sample standard deviation to two decimal places.
...
[**Q17.**]{style="color:green;"} Compare the standard deviation of the `Age` variable in the L1 and L2 datasets. What can you conclude on the basis of this comparison?
...
You have successfully completed [`r checkdown::insert_score()` out of 13 quiz questions]{style="color:green;"} in this chapter.
...
-   [ ] Recognise the characteristics of a normally distributed variable (@sec-Normal)
-   [ ] Interpret and calculate the interquartile range in `R` (@sec-IQR)
-   [ ] Interpret the distribution of a numeric variable summarised as a boxplot (@sec-IQR)
-   [ ] Interpret and calculate the standard deviation (@sec-SD)

In Chapter 10, you will learn how to make beautiful plots (including histograms and density plots) in `R` using `ggplot2`. But, first, we need to learn about data wrangling to prepare our data for data visualisation and comparative analyses across different variables and datasets. Are you ready? 🤓
