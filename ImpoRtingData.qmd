...
We will begin by creating a new `R` script in which we will write the code necessary to import the data from @DabrowskaExperienceAptitudeIndividual2019's study in `R`. To do so, from the Files pane in RStudio, click on the analysis folder to open it and then click on the 'New Script' icon in the top-left corner of the Files pane and select 'R Script'. This will open a new, empty `R` script in your Source pane. It is best to always begin by saving a newly created file. Save this empty script with a computer- and human-friendly file name such as as `1_DataImport.R` (@sec-FileNaming). It should now appear in your analysis folder in the Files pane.

```{r include = FALSE}

#To do for Jan: Add screenshot of Files pane showing an empty "analysis" folder and with red circle around the "new script" icon.
...
```{r}
here("data", "Dabrowska_2018_L1_data.csv")
```
