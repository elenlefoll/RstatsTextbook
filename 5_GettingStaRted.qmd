...
Notice that, in this erroneous line of code, we have (intentionally) forgotten to include the final bracket. As a result, after you hit "Enter", the Console output shows a "`+`" instead of the result of the mathematical operation (see @fig-ConsoleIncompleteFunction). The "`+`" indicates that the line is incomplete and therefore cannot be interpreted yet. The "`+`" means that `R` is asking you to complete your line of code.
...
There are two ways to fix this. The first method is to complete the line of code directly in the Console. In this case, this means adding the closing bracket "`)`" after the "`+`" and hitting "Enter" again. Now that the line has been completed, `R` is able to interpret it as a valid `R` command and therefore outputs the expected result.
