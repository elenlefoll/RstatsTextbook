...
- Keep your cool when errors pop up! [😎]{.content-visible when-format="html"}.
:::
...
::: {.content-visible when-format="html"}
[This is what you did in @sec-installingR when you tested that *RStudio* was working properly (using the command: `plot(1:10)`).]{.aside}
:::
...
::: {#fig-ConsoleInput layout="[[40,-1,40], [100]]" layout-valign="center"}
![Inputting numbers](images/ConsoleInputNumbers.png){#fig-ConsoleInputNumbers fig-alt="Screenshot of RStudio showing the Console with the equation 13 + 0.5 = 13.5."}

![Inputting strings](images/ConsoleInputStrings.png){#fig-ConsoleInputStrings fig-alt="RStudio screenshot showing the input of two strings in the console"}
...
Try first inputting a single word and then an entire sentence in the Console as in @fig-ConsoleInputStrings.

::: {.callout-warning collapse="false"}
Word processors such as Microsoft Word, Pages, and LibreOffice Writer should *not* be used to write, edit, or share `R` scripts. Among other problems, such programmes frequently enable an automatic 'smart quotes' feature, which silently replaces straight quotation marks (`"`) with curly ones (“ and ”). These correspond to different Unicode characters, which `R` does not recognise as valid string delimiters. Hence, curly quotes result in broken code that returns errors, e.g.:
...
![Using the `R` Console as a calculator](images/ConsoleInputMathematicalOperations.png){#fig-ConsoleInputMathematicalOperations fig-alt="RStudio screenshot showing the console with the input of multiple mathematical operations." width="60%"}

:::: {.content-visible when-profile="OER"}
::: {.callout-tip collapse="false"}
#### Your turn! {.unnumbered}

[**Q5.2**]{style="color:green;"} Compare `13*13` with `13 * 13`. What is the difference in the output?
...
[**Q5.3**]{style="color:green;"} Try entering `13^2` in the Console. What does the `^` (caret) operator do?

```{r}
#| echo: false
#| label: "Q5.3"

check_question("The ^ operator performs an exponentiation operation, here 13 is raised to the power of 2.", 
               options = c("The ^ operator performs an exponentiation operation, here 13 is raised to the power of 2.", "The ^ operator calculates the modulus of a number, here of 13 with 2 as the base.", "The ^ operator creates a vector, here with 13 occurrences of the integer 2."), 
               type = "radio",
               button_label = "Check answer", 
               q_id = "Q5.2",
               random_answer_order = TRUE,
...
![Created object in the Environment pane](images/ObjectCreation.png){#fig-ObjectCreation fig-alt="RStudio screenshot showing that an object is now stored in the session environment in the left-hand pane." width="70%"}
...
:::: {.content-visible when-format="html"}
::: callout-tip
If you're feeling lazy or simply want to avoid making a typo, you can type just the first few letters of an object name and then press the key `Tab` key {{< kbd mac=⇥ win=↹ linux=↹ >}}. *RStudio* will then give you a drop-down menu with possible options. Select the one you want by clicking on it or pressing "Enter".
:::
::::

::: {layout="[[40,-1,40], [100]]" layout-valign="center"}
![Calling up stored objects in the Console to view their content](images/ShowObjects.png){#fig-ShowObjects fig-alt="RStudio screenshot showing two stored objects printed in the console."}

![Using the `class()` function](images/ShowObjectClass.png){#fig-ShowObjectClass fig-alt="RStudio screenshot showing the object types with the class() function from two stored objects."}
:::

### Object types

These two objects are of different types. We can use the `class()` function to find out which type of object an object is (see @fig-ShowObjectClass).
...
![Environment pane showing all of the objects currently stored in the `R` session environment](images/MultipleObjectsEnvironmentPane.png){#fig-MultipleObjectsEnvironmentPane fig-alt="RStudio screenshot showing the environment pane with six stored objects." width="70%"}
...
check_question(c("1TopWord", "BestWordEver!", "my-favourite-word", "top word", "Agnes'Fav.Word"),
options = c("1TopWord", "BestWordEver!", "my-favourite-word", "top word", "Agnès.Favourite.Word", "Top1Word", "Agnes'Fav.Word"),
type = "checkbox",
button_label = "Check answer", 
q_id = "Q5.4",
random_answer_order = TRUE,
...
Earlier on, you created an object called `word2` which contained the string "cheerful". But, by running this new line of code, "cheerful" has been replaced by the string "surprised" - with no warning that you were about to permanently delete "cheerful"! `r emoji("astonished")`
...
3.  Use the keyboard shortcut {{< kbd mac=Cmd-Shift-N win=Ctrl-Shift-N linux=Ctrl-Shift-N >}}.
...
To send a line of code to the Console (also referred to as "executing" or "running" code), select the line that you want to execute, or place your mouse cursor anywhere within that line and then click on the "Run" button (in the top-right corner of the pane, see @fig-NewScript) or use the keyboard shortcut {{< kbd mac=Cmd-Enter win=Ctrl-Enter linux=Ctrl-Enter >}}. This shortcut is worth learning, as it will save you a lot of time and effort in the long term.

Try out these two options to run the three lines of code of your script, then check that:

a.  you are seeing the result of the mathematical operation in the Console output and
b.  two objects have been added to your environment.
...
### Saving an `.R` script {#sec-SavingRScripts}
...
2.  Click on the save icon `r emoji("floppy_disk")`, or
3.  Use the keyboard shortcut {{< kbd mac=Cmd-S win=Ctrl-S linux=Ctrl-S >}}.
...
The relational operators `==` and `!=` can also be used with character objects. Find out how they work by first creating a new character object with a word that was added to the 2025 edition of the popular French dictionary *Petit Larousse*.[^5_gettingstarted-1]

[^5_gettingstarted-1]: In the [online version](https://www.larousse.fr/dictionnaires/francais/%C3%A9cogeste/192366) of the *Larousse* dictionary, *écogeste* is defined as:

    > Habitude simple que chacun peut adopter dans la vie de tous les jours afin de protéger l'environnement (réduire et trier ses déchets, ne pas laisser couler l'eau, éteindre la lumière, acheter ses aliments en vrac, etc.) \[Simple habits that anyone can incorporate in their daily life to protect the environment (reducing and sorting waste, turning off lights and taps, buying non-packaged food, etc.)\] (translation ELF; last accessed 5 May 2026).
...
Earlier in the chapter, we created the following `R` character string object:
...
## Dealing with errors `r emoji("cursing_face")` {#sec-Errors}
...
![The emotional rollercoster of debugging by [\@allison_horst](https://allisonhorst.com/allison-horst) (CC BY 4.0)](images/AHorst_DebuggingMonsters.png){#fig-DebuggingMonster fig-alt="A cartoon of a fuzzy round monster face showing 10 different emotions experienced during the process of debugging code. The progression goes from (1) “I got this” - looking determined and optimistic; (2) “Huh. Really thought that was it.” - looking a bit baffled; (3) “...” - looking up at the ceiling in thought; (4) “Fine. Restarting.” - looking a bit annoyed; (5) “OH WTF.” Looking very frazzled and frustrated; (6) “Zombie meltdown.” - looking like a full meltdown; (7) (blank) - sleeping; (8) “A NEW HOPE!” - a happy looking monster with a lightbulb above; (9) “insert awesome theme song” - looking determined and typing away; (10) “I love coding” - arms raised in victory with a big smile, with confetti falling." width="80%"}
...
![Incomplete function in console](images/ConsoleUncompleteFunction.png){#fig-ConsoleIncompleteFunction fig-alt="RStudio screenshot showing the console with an incomplete function and a red ellipse around the +-symbol." width="60%" fig-pos="H"}
...
Given that we will mostly be working in scripts to ensure that our analyses are reproducible (see @sec-LiteRateProgramming), let's now generate this error from an `.R` script. To do so, copy and paste the erroneous line of code in your `.R` script and try to run it by either clicking on the "Run" icon or using the shortcut {{< kbd mac=Cmd-Enter win=Ctrl-Enter linux=Ctrl-Enter >}}.
...
The plus-situation arises again, but we will now solve it using the second method. Head over to the Console and place your cursor next to the `+`. This time, instead of completing the line by adding a closing bracket, press the Escape key {{< kbd mac=Esc win=Esc linux=Esc >}} on your keyboard. This will cancel the incomplete line of code. Then, you can add the missing `)` in your script and rerun the newly completed line of code from the Source pane.
...
Learning to make sense of error messages is a very important skill that, like all skills, takes practice. Most errors are very easy to fix, if you keep your cool. In fact, 90% of errors are simply typos[^5_gettingstarted-2], so really nothing worth stressing about!

[^5_gettingstarted-2]: I must confess that I made this number up. I don't have any reliable statistic on this, but it's fair to say that it's a very large proportion!
...
Debugging is an unavoidable part of writing code. If you're stuck and starting to feel frustrated, the best thing you can usually do is to take a short break (see @fig-TakeABreak)!

![Time to take a break? (artwork by [\@allison_horst](https://allisonhorst.com/allison-horst) CC BY 4.0)](images/AHorst_ErrorMonster.png){#fig-TakeABreak fig-alt="A frustrated little monster sits on the ground with his hat next to him, saying \"I just need a minute.\" Looking on empathetically is the R logo, with the word \"Error\" in many different styles behind it." width="50%"}
...
It's time to complete this chapter's [tasks and quizzes](https://elenlefoll.github.io/RstatsTextbook/quizzes/5_quiz.html) and ensure that you are ready to get started with `R` in earnest!
:::
...
If that's the case, you're ready to go through @sec-ImportingData to learn how to import research data into `R` so that, in the following chapters, you can analyse real-life linguistics data!
