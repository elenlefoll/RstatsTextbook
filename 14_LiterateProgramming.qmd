...
[^14_literateprogramming-1]: Although there is ground for optimism here because more and more linguists and education scholars are beginning to make their data open in repositories such as the Open Science Framework (OSF), IRIS, and TROLLing (see @sec-Sharing).
...
[**Q13.1**]{style="color:green;"} What is meant by the replication crisis?[^14_literateprogramming-2]
...
Your supervisor or colleague may request a **Microsoft Word** document and, thankfully, this is no problem. You can change the rendering format to a `.docx` file by changing the format option in your **YAML header**:
...
With this format option, rendering your Quarto document will automatically generate a `.docx` file that includes your text, any code that you wanted to show in your document, and all of the code outputs that you wanted to share, such as your statistics, graphs, and tables.

Many (but not all!) of the formatting options available for HTML also work for the `.docx` format:
...
Note that any options that are not available in the specified format are ignored without a warning or error message.

::: callout-warning
#### Not rendering code chunks in specific formats

Dynamic code outputs, such as the **interactive** {plotly} graph displayed in @fig-scatterplot-plotly, cannot be meaningfully rendered to **static formats**, such as Microsoft Word documents or PDF. Attempting to do so can cause rendering errors such as:
...
In this chapter, we've only just scratched the surface of what's possible in Quarto. The Quarto documentation is very detailed and well worth exploring to find out what else you can do in Quarto: <https://quarto.org/docs/guide/>. From books to blogs and interactive dashboards, the world's your oyster! 🚀
