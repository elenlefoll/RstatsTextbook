...
As we saw in @sec-OpenScience, it is good practice to share both the data and materials associated with research studies so that others can reproduce and replicate[^6_importingdata-1] the research.

[^6_importingdata-1]: For more on reproductions and replications, see @sec-Reproducibility.
...
Follow the DOI[^6_importingdata-2] link associated with the article referenced above and read the abstract to find out what the study was about. Note that you do not need to have an institutional or paid access to the full paper to able to read the abstract.

[^6_importingdata-2]: Digital Object Identifiers (DOI) are alpha-numeric strings that can be assigned to publications, preprints, datasets, and all other kinds of research materials with the aim of facilitating the exchange of information on digital networks and avoiding dead links [@parsonsCommunitysourcedGlossaryOpen2022].

:::: {.content-visible when-profile="OER"}
::: {.callout-tip collapse="false"}
#### Your turn! {.unnumbered}

[**Q6.1**]{style="color:green;"} What types of data were collected as part of @DabrowskaExperienceAptitudeIndividual2019?
...
[**Q6.2**]{style="color:green;"} On average, how did the English L2 speakers perform compared to the native speakers in @DabrowskaExperienceAptitudeIndividual2019?
...
#### What's a good place to save these files? `r emoji("thinking")`

If you haven't already done so, now is a good time to create a folder in which you save everything that you create while learning from this textbook. This folder could be called something along the lines of `DataAnalysisR` or `2026_Data-Analysis-R` (see @sec-FileNaming). Then, within this folder, I recommend that you create a subfolder called `Dabrowska2019` (note how I have not included the "ą" character in the folder name as this could cause problems on some operating systems), and within this folder, create another subfolder called `data`. This is the folder in which you can save these two data files.
:::
...
The file `L1_data.csv` contains data about the study's L1 participants. It is a delimiter-separated values (DSV) file (see @sec-DSV). The first five lines of the file are printed below. Note that this is a very wide table as it contains many columns.
...
To create a new Project, you have two options. In *RStudio*, you can select 'File', then 'New Project...' (see @fig-RStudioNewProject a). Alternatively, you can click on the Project button in the top-right corner of your *RStudio* window and then select 'New Project...' (see @fig-RStudioNewProject b).

![Create a new project in *RStudio*](images/RStudioNewProject.png){#fig-RStudioNewProject fig-alt="Screenshot of the *RStudio* interface with two highlighted sections for creating a new project. The first highlight (a) is on the 'File' menu in the top left corner, where the option 'New Project...' is circled. The second highlight (b) is in the top right corner, where the option 'New Project...' in a dropdown menu is circled. Both options provide a way to start a new project in RStudio." width="80%"}
...
Clicking on this option will open up a new window (@fig-NewProjectWindow). Click on 'Browse...' to navigate to the folder where you intend to save all your work related to @DabrowskaExperienceAptitudeIndividual2019. If you followed my suggestions earlier on, this would be a folder called something along the lines of `Dabrowska2019`. Once you have selected the correct folder, select the option 'Open in a new session' and then click on 'Create Project'.

![New project window](images/RStudioNewProjectDialogue.png){#fig-NewProjectWindow fig-alt="New Project Wizard for Create Project from Existing Directory R Project. It shows the working directory: .../Documents/DataLiteracyTextbook/Dabrowska2019. There are three buttons: Browse..., Create Project and Cancel" width="40%"}
...
::: {#fig-filenames layout="[[50,-1,50], [100]]" layout-valign="center"}
![Launching a Project from the File Finder](images/ProjectFileFinder.png){#fig-ProjectFileFinder fig-alt="Screenshot of the project folder in the MacOS Finder with the .Rproj-file highlighted. To launch the project in a new session, the corresponding .Rproj-file can be double-clicked."}

![Launching a Project from RStudio](images/RStudioLaunchProject.png){#fig-RStudioLaunchProject fig-alt="Screenshot of the dropdown menu under the 'Project: (None)' in RStudio, displaying options such as 'New Project...', 'Open Project...', 'Open Project in New Session...', and 'Close Project.' The 'Open Project in New Session...' option is circled in red."}

Two different ways to open an R Project.
:::

## Working directories {#sec-WorkingDirectories}

The folder in which the `.Rproj` file was created corresponds to your project's **working directory**. Once you have opened a Project, you can see the path to your project's working directory at the top of the Console pane in RStudio. The Files pane should also show the content of this directory as in @fig-ContentsProj.

![Contents of the project folder as displayed by *RStudio*](images/WorkingDirectory.png){#fig-ContentsProj fig-alt="Screenshot of the Files pane of RStudio showing the contens of a folder entitled 'Dabrowska2019'. It contains an R Project file called 'Dabrowska2019.Rproj', a folder called 'analysis' and another folder called 'data'." width="40%"}
...
::: {#nte-here .callout-important collapse="false"}
#### "Hell is empty, and all the devils are here." `r emoji("smiling_imp")`
...
To make your projects more robust to such issues, I strongly recommend working with the [{here}](https://here.r-lib.org/) package in addition to R Projects. You will first need to install the package:
...
Much like wearing a helmet for extra safety (@fig-HelmetHere), {here} makes the paths that you include in your code far more robust. In other words, they are far less likely to fail and break your code when you share your scripts with your colleagues, or run them yourself from different directories or operating systems. For more reasons to use {here}, check out Malcolm @barrettWhyShouldUse2018's blog post "[Why should I use the here package when I'm already using projects?](https://malco.io/articles/2018-11-05-why-should-i-use-the-here-package-when-i-m-already-using-projects)".
:::

Although a fairly common way of working with data in `R`, using `setwd()` (see @sec-HardcodingFilePaths) is dangerous and will, sooner or later, cause you and/or your colleagues some nasty accidents (see @fig-HelmetHere). A combination of using R projects (`.RProj`) and the {here} package as described above is much safer!

![Artwork CC BY 4.0 [\@allison_horst](https://twitter.com/allison_horst)](images/AHorst_RProjHere.png){#fig-HelmetHere fig-alt="A cartoon of a cracked glass cube looking frustrated with casts on its arm and leg, with bandaids on it, containing 'setwd', looks on at a metal riveted cube labeled 'R Proj' holding a skateboard looking sympathetic, and a smaller cube with a helmet on labeled 'here' doing a trick on a skateboard." width="70%"}

## Import errors and issues `r emoji("pleading_face")` {#sec-ImportingErrors}
...
[**Q6.11**]{style="color:green;"} You have saved the two datasets to your local `R` environment as `L1.data` and `L2.data`. What kind of `R` objects are `L1.data` and `L2.data`? You can find out by using the command `class()`. It takes the name of the object as its only argument.
...
q_id = "Q6.12", 
random_answer_order = TRUE,
right = "That's right, well done! We'll come back to these data in future chapters but, for now, let's briefly consider other data formats with which we can work in R.",
...
In the [**Your turn!**]{style="color:green;"} section in @sec-DSVLibreOffice, you downloaded and examined a DSV file with a `.txt` extension that was separated by tabs: `offlinedataLearners.txt` from @schimkeFirstLanguageInfluence2018.
...
![Extract of data file AJT_raw_scores_L2.csv from](images/BusterudEA2023_data_snapshot.png){#fig-BusterudEAData fig-alt="The image shows a table from the data file AJT_raw_scores_L2.csv by Busterud et al. It includes rows numbered 261 to 274 with columns for ID, L3, Years of L3, Gender, L3 selfasses, L3 grade, L2 selfasses, and L2 grade. All entries in L3, Years of L3, and Gender are 2, 4, and 4, respectively. Values for L3 selfasses range from 1 to 2,5, L3 grade from 2 to 5, L2 selfasses from 2 to 6, and L2 grade from 3 to 6." width="80%"}

If you look carefully, you will also see that this dataset has some empty cells. These data can be downloaded from <https://doi.org/10.18710/JBMAPT>. It is delivered with a README text file. It is good practice to include a README file when publishing datasets or code and, as the name suggests, it is always a good idea to actually read README files! `r emoji("upside_down_face")` Among other things, this particular README explains that, in this dataset: "Missing data are represented by empty cells."
...
1.  Install just this package
2.  Or install the full tidyverse (this will take a little longer).
3.  Load the library.
...
```{r eval=TRUE}
L1.data <- read.csv(file = "data/L1_data.csv", # <1>
                    header = TRUE, 
                    quote = "\"")

class(L1.data)

L1.data <- read_csv(file = "data/L1_data.csv", # <2> 
                    col_names = TRUE, 
                    quote = "\"")
...
1.  Import `.csv` file using the base `R` function `read.csv()`
2.  Import `.csv` file using the {readr} function `read_csv()`
...
To import `.ods` files, as used natively by open-source text processing software such as LibreOffice Calc (which you should have installed in @sec-OpenSource) and OpenOffice Calc, you can install the [{readODS}](https://www.rdocumentation.org/packages/readODS/versions/2.3.0) package and use its `read_ods()` function:

```{r eval = FALSE}
install.packages("readODS") # <1>
remotes::install_github("ropensci/readODS") # <2>

library(readODS) # <3>

MyLibreOfficeData <- read_ods("data/MyLibreOfficeTable.ods", # <4>
                            sheet = 1,
                            col_names = TRUE,
                            na = "NA")
...
1.  Install from CRAN. This is the safest option.
2.  Alternatively, install the development version from Github.
3.  Load the library.
4.  Import the first tab of your `.ods` spreadsheet, ensuring that the first row is processed as the column headers and that `NA` are recognised as such.

### Microsoft Excel

Various packages can be used to import Microsoft Excel file formats, but the simplest is [{readxl}](https://readxl.tidyverse.org/), which is also part of the {tidyverse} (@sec-tidyverse). It allows users to import data in both `.xlsx` and the older `.xls` format. 

```{r eval = FALSE}
install.packages("readxl") # <1>
remotes::install_github("tidyverse/readxl") # <2>

library(readxl) # <3>

MyExcelData <- read_excel("data/MyExcelSpreadsheet.xlsx", # <4>
                            sheet = 1,
                            col_names = TRUE,
                            na = "NA")
...
1.  Install from CRAN (safest option).
2.  Or install the development version from Github.
3.  Load the library.
4.  Import the first tab of your Excel spreadsheet, ensuring that the first row is processed as the column headers and that `NA` are recognised as such.
...
However, if you want to directly import your data from Google Sheets and be able to dynamically update the analyses that you conduct in `R` even as the input data are amended on Google Sheets, you can use the [{googlesheets4}](https://googlesheets4.tidyverse.org/) package (which is part of the {tidyverse}):

```{r eval = FALSE}
install.packages("googlesheets4") # <1>
remotes::install_github("tidyverse/googlesheets4") # <2>

library(googlesheets4) # <3>

MySheetsData <- read_sheet("https://docs.google.com/spreadsheets/YourURL", # <4>
                            sheet = 1,
                            col_names = TRUE,
                            na = "NA")

MySheetsData <- read_sheet("YourGoogleSheetsID", # <5>
                            sheet = 1,
                            col_names = TRUE,
                            na = "NA")
...
If you've recently switched from working in SPSS, SAS, or Stata (or are collaborating with someone who uses these programmes), it might be useful to know that you can also import the data files created by programmes directly into `R` using the [{haven}](https://haven.tidyverse.org/) package.

```{r eval = FALSE}
install.packages("haven") # <1>
remotes::install_github("tidyverse/haven") # <2>

library(haven)  # <3>

MySASData <- read_sas("MySASDataFile.sas7bdat")  # <4>

MySPSSDataFile <- read_sav("MySPSSDataFile.sav") # <5>

MyStataData <- read_dta("MyStataDataFile.dta") # <6>
...
2.  Or install the development version from Github.
3.  Load the library.
4.  Import a SAS data file.
5.  Import a SPSS data file.
6.  Import a Stata data file.
...
In addition, fellow linguists are constantly developing new packages to work with file formats that are specific to our discipline. In the spirit of Open Science (see @sec-OpenScholarship), many are making these packages available to the wider research community by releasing them under open licenses. For example, linguistics M.A. students Katja Wiesner and Nicolas Werner wrote and published an `R` package called [rELAN](https://github.com/relan-package/rELAN/?tab=readme-ov-file) to facilitate the import of the `.eaf` files generated by the linguistics annotation software [ELAN](https://archive.mpi.nl/tla/elan) [@lausbergCodingGesturalBehavior2009] as part of a seminar project supervised by Dr. Fahime (Fafa) Same at the University of Cologne.

:::: {.content-visible when-profile="OER"}
::: {.callout-tip collapse="false"}
#### Your turn! {.unnumbered}

[**Q6.13**]{style="color:green;"} Which of these files types can be imported into an `R` session?

```{r}
#| echo: false
#| label: "Q6.13"
library(checkdown)
check_question(c("Microsoft Excel files",
                 ...
                 "Stata data files",
                 "ELAN files"), 
               options = c("Microsoft Excel files",
                 "LibreOffice Calc files",
                 "OpenOffice Calc files",
                 "SPSS data files",
                 "Google Sheets documents",
...
random_answer_order = TRUE,
q_id = "Q6.13", 
right = "That's right! There are packages that allow us to easily import all of these types of data files (and many more!) into `R`.",
wrong = "Are you sure it's not possible to import any more file types from this list?")
```
:::
::::
...
### Hardcoding file paths in `R` scripts `r emoji("woozy_face")` {#sec-HardcodingFilePaths}
...
Instead of creating an R Project to determine a project's working directory (as we did in @sec-RProject), it is possible to begin a script with a line of code that sets the **working directory** for the script using the function `setwd()`, e.g.:

```{r eval = FALSE}
setwd("/Users/lefoll/UzK/RstatsTextbook/Dabrowska2019")
```
...
### Importing data using RStudio's GUI `r emoji("roll_eyes")` {#sec-ImportingGUI}
...
![Importing a file from RStudio's File pane](images/RStudioImportDataset.png){#fig-RStudioImportDataset fig-alt="Screenshot of the Files pane in *RStudio* displaying the options after clicking on a CSV file. The option 'Import Dataset...' is highlighted. Clicking on this will open the 'Import Text Data' dialog for importing this file." width=40%}
...
![*RStudio*'s "Import Text Data" dialogue](images/RStudioReadrImportTextDataDialogue.png){#fig-RStudioReadrImportTextDataDialogue fig-alt="Screenshot of the 'Import Text Data'-dialogue showing the import settings of a the file 'L1_data.csv'. It shows the 'File URL' of the selected file to be imported, a 'Data Preview', the 'Import Options' and the 'Code Preview'." width="90%"}
...
You have successfully completed [`r checkdown::insert_score()` out of 13 questions]{style="color:green;"} in this chapter.
:::

::: {.content-visible when-format="pdf"}
### Check your progress {.unnumbered}

It's time to complete this chapter's [tasks and quizzes](https://elenlefoll.github.io/RstatsTextbook/quizzes/6_quiz.html)! These tasks are crucial to be able to continue with the following chapters.
:::
...
-   [ ] Import files with single and multiple tabs from spreadsheet software (@sec-SpreadsheetSoftware)
-   [ ] Find out how to import data in other file formats (@sec-SPPS - @sec-OtherFileFormats)

Now it's time to start exploring research data in `R`! In @sec-VaRiablesAndFunctions you will learn how to work with in-built `R` functions to find out more about the DSV data from @DabrowskaExperienceAptitudeIndividual2019 that you imported in this chapter.
