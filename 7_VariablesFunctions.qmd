...
Like many of the relational operators we saw in @sec-RelationalOperators, the `R` pipe is a combination of two symbols, the computer pipe symbol `|` and the right angle bracket `>`. Don't worry if you're not sure where these two symbols are on your keyboard as *RStudio* has a handy shortcut for you: {{< kbd mac=Cmd-Shift-M win=Ctrl-Shift-M linux=Ctrl-Shift-M >}} [^7_variablesfunctions-2] (see @fig-Pipes). I strongly recommend that you write this shortcut on a prominent post-it and learn it asap, as you will need it a lot when you are working in `R`!
...
### Check your progress {.unnumbered}
It's time to complete this chapter's [tasks and quizzes](https://elenlefoll.github.io/RstatsTextbook/7_quiz.html)! Good luck! `r emoji("four_leaf_clover")`
::::
...
You are now ready to some statistics in `R`! In @sec-DescRiptiveStats, we begin with descriptive statistics.
