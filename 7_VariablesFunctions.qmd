...
In this chapter and the following, all analyses are based on data from:
...
You will only be able to reproduce the analyses and answer the quiz questions from this chapter if you have successfully imported the two data sets from @DabrowskaExperienceAptitudeIndividual2019. To import the data, follow the instructions from @sec-RProject to @sec-ImportingDataCSV and complete Task 1.
:::
...
-   Use base `R` functions to inspect tabular data
-   Inspect and access individual variables
-   Access individual data points
-   Use simple base `R` functions to inspect variables
-   Change the default arguments of functions
-   Combine functions using two methods

## Inspecting tabular data

In @sec-ImportingErrors, we saw that we can use the `View()` function to display tabular data in a format that resembles that of a spreadsheet programme.

This particular dataset is both long and wide so you will need to scroll in both directions to view all the data. RStudio also provides a filter option and a search tool (see @fig-ViewData1). Note that both can only be used for visual inspection. You cannot alter the dataset in any way using these tools.
...
[**Q1.**]{style="color:green;"} The `View()` function is more user-friendly than attempting to examine the full table in the Console. Try to display the full L2.dataset in the Console by using the command `L2.data` which is shorthand for `print(L2.data)`. What happens?

```{r echo=FALSE, results="asis"}
check_question("R only displays the first 32 rows and the columns are not aligned because the Console window is not wide enough.", 
               options = c("R only displays the first 32 rows and the columns are not aligned because the Console window is not wide enough.", 
...
right = "That's right. The printed message \"[ reached 'max' / getOption(\"max.print\") -- omitted 45 rows ]\" is not an error, but merely a message informing us that not all rows could be printed.",
wrong = "No, that's not what is happening. Try scrolling up within the Console window to better understand what is going on.")
```

 
:::

In practice, it is often useful to printing subsets of a dataset in the Console to quickly check the sanity of the data. To do so, we can use the function `head()` that prints the first six rows of tabular data.
...
[**Q2.**]{style="color:green;"} Six is the default number of rows printed by the `head()` function. Have a look at the function's help file using the command `?head` to find out how to change this default setting. How would you get `R` to print the first 10 lines of `L2.data`?
...
button_label = "Check answer",
random_answer_order = TRUE,
right = "That's right! In fact, three of the above options will work. Try them out to work out which ones they are!",
wrong = "No, that won't display the first 10 rows of the dataset. Try it out for yourself!")
```
...
In tidy data formats (see \*\*), each row corresponds to one observation and each column to a variable. Each cell, therefore, corresponds to a single data point, which is the value of a specific variable (column) for a specific observation (row). As we will see in the following chapters, this data structure allows for efficient and intuitive data manipulation, analysis, and visualisation.

The `names()` functions returns the names of all of the columns of a data frame. Given that the datasets from @DabrowskaExperienceAptitudeIndividual2019 are 'tidy', this means that `names(L1.data)` returns a list of all the variables in the L1 dataset.

```{r}
names(L1.data)
```

A useful way to get a quick and informative overview of the variables of a large dataset is to use the function `str`, which was mentioned in @sec-ImportingErrors, and which returns the "internal structure" of any `R` object. It is particular useful for large datasets with many variables.

```{r}
str(L1.data)
```

At the top of its output, the function `str(L1.data)` first informs us that `L1.data` is a **data frame** object, consisting of 90 observations (i.e. rows) and 31 variables (i.e. columns). Then, it returns a list of all of the variables included in this data frame. Each line starts with a `$` sign and corresponds to one variable. First, the name of the variable (e.g. `Occupation`) is printed, followed by the variable's `R` data type (e.g. `chr` for a character string variable), and then its values for the first few rows of the table (e.g. we can see that the first participant in this dataset was a "Student" and the second a "Student/Support Worker").
...
[**Q3.**]{style="color:green;"} Use the `str()` function to examine the internal structure of the L2 dataset. How many variables are there in the L2 dataset?
...
[**Q4.**]{style="color:green;"} Which of these variables can be found in the L2 dataset, but not the L1 one?
...
button_label = "Check answer",
random_answer_order = TRUE,
right = "That's right! And there are more variables that are specific to the L2 dataset but which are not listed here.",
...
[**Q5.**]{style="color:green;"} Which type of `R` object is the variable `Arrival` stored as?
...
[**Q6.**]{style="color:green;"} How old was the third participant listed in the L2 dataset when they first moved to an English-speaking country?
...
We can call up individual variables within a data frame using the `$` operator. This displays all of the participants' values for this one variable. As shown below, this works for any type of variable.
...
[**Q7.**]{style="color:green;"} Use the index operators to find out the native language of the 26^th^ L2 participant.
...
button_label = "Check answer",
right = "Brawo! 🎉",
wrong = "That's incorrect. 😔 Use the command `L2.data$NativeLg[26]` to find out which it is.")
```

 

[**Q8.**]{style="color:green;"} Which command(s) can you use to display only the Gender, Occupation, Native language, and Age of the last participant listed in the L2 dataset?
...
We know from our examination of the L1 dataset from @DabrowskaExperienceAptitudeIndividual2019 that it includes 90 English native speaker participants. To find out their average age, we could add up all of their ages and divide the sum by 90.
...
This already looks much better, but it's still less than ideal: What if we decided to exclude some participants (e.g., because they did not complete all of the experimental tasks)? Or decided to add data from more participants? In all of these cases, 90 will no longer be the correct denominator to calculate their average age! That's why it is better to work out the denominator by computing the total number of values in the variable of interest. To this end, we can use the `length()` function, which returns the number of values in any given vector.
...
[**Q9.**]{style="color:green;"} How does the average age of the L2 participants in @DabrowskaExperienceAptitudeIndividual2019 compare to that of the L1 participants?
...
Whilst the first three return exactly the same output, the third returns an error because the argument names are not specified and are not in the order specified in the function's help file. To avoid making errors and confusing your collaborators and/or future self, it's good practice to explicitly name all the arguments except the most obvious ones.
...
[**Q10.**]{style="color:green;"} Based on your observations, what does the `round()` function do?
...
button_label = "Check answer",
random_answer_order = TRUE,
right = "That's right. The mean age for L2 participants is 32.72.",
wrong = "Are you sure? Try out the `round()` function with other numbers to find out more.")
```

 

[**Q11.**]{style="color:green;"} Check out the 'Usage' section of the help file on the `round()` function to find out how to round the `Vocab` values in the L1 dataset to two decimal places. How can this be achieved?
...
Combining functions is where the real fun starts with programming! For example, if we want to compute participant's average age to two decimal places, we need to combine the `mean()` function and the `round()` function. We could achieve this in two steps.
...
![A schematic representations of one function, two nested functions and three nested functions, each with two arguments](images/NestedFunctions.png){#fig-NestedFunctions fig-alt="The text reads like this: function(argument 1, argument 2) functionB(functionA(argument 1a, argument 2a), argument1b, argument2b) functionC(functionB(functionA(argument 1a, argument 2a), argument1b, argument2b), argument1c, argument2c)."}

::: callout-tip
### Time to think!

Consider the two sets of nested functions below. Without running them, can you tell which of the two lines of code will output the square root of L1 participant's average age to two decimal places?
...
The first line actually outputs `6.126989`, which has more than two decimal places! This is because `R` interprets the functions from the inside out: first, it calculates the mean value, then it rounds that off to two decimal places, then this outputs the square root of that value. The second line, in contrast, does the rounding operation as the last step. Note that, in these two lines of code, the brackets around the argument `digits = 2` are also located in different places. It is very easy to make bracketing errors when writing code and especially so when nesting functions (see @fig-NestedFunctions).
:::

### Piped Functions

If you found all these brackets overwhelming: fear not! There is a second method for combining functions, which is more convenient and much easier to decipher. It involves the pipe operator, which in `R` is `|>`.[^7_variablesfunctions-1]
...
[**Q12.**]{style="color:green;"} Using the `R` pipe operator, calculate the average mean age of the L2 participants and round off this value to two decimal places. What is the result?
...
::: {.callout-note collapse="true"}
#### Click here for a detailed answer to Q13

They are lots of ways to tackle question 15. Here is one approach:
