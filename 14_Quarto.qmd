...
::: callout-note
A standalone version of this chapter is available as [Quarto for reproducible research workflows and academic publishing: A step-by-step tutorial](https://elenlefoll.quarto.pub/quarto4research/) [@lefollQuartoReproducibleResearch2026].
:::
...
``` {.bib .unnumbered filename="references.bib"}
@article{
...
::: callout-note
#### Don't git muddled up! `r emoji("upside_down_face")`
...
## Rendering formats and options {#sec-Publishing}
...
::: {#fig-MSWordWarnings layout-ncol="2" width="60%"}
![](images/Word_popup.png){#fig-WordRecovery fig-alt="Dialog menu from Microsoft Word that reads: Word found unreadable content in LearningQuarto3.docx. Do you want to recover the contents of this document? If you trust the source of this document, click Yes. Below the message there are two buttons: Yes and No."}

![](images/Word_repairs.png){#fig-WordRepairs fig-alt="Dialog menu from Microsoft Word entitled: Repairs. It reads: Word detected and repaired the following errors. To view each repair, select it in the list, then click Go To. Save the document to make the repairs permanent. Table Properties 1 Table Properties 2 Table Properties 3 etc. There is one button: Close"}
...
### PDF via [$\LaTeX$]{.conditional when-format="html"}[\LaTeX]{.conditional when-format="pdf"} {#sec-PDF}

It is also possible to render Quarto documents to PDF; however, this requires you to have [LaTeX](https://en.wikipedia.org/wiki/LaTeX) installed on your computer. Alternatively, you can use [Typst](https://quarto.org/docs/output-formats/typst.html), a new open-source markup-based typesetting system designed to be as powerful as [$\LaTeX$]{.conditional when-format="html"}[\LaTeX]{.conditional when-format="pdf"}, but easier to use.
