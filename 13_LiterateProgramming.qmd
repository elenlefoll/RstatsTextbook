...
<!--#  @Elen: Did you forget to insert an image here? -->
...
![](images/Quarto_Source-Visual.png){#fig-QuartoModes}
...
![](images/LearningQuarto_intro.png){#fig-QuartoIntro}
:::
...
To embed an image from an external file, you can use the "Insert" menu in RStudio's **Visual** editor and select "Figure / Image" (see @fig-InsertingImage). This will open up a menu where you can select the image that you want to insert, as well as add **alt-text** (see @sec-Alt-text) and a **caption**. The easiest way to adjust the size of an embedded image is to click on the image and then adjust the **size** of the image with the blue circle in the bottom-right corner of the image (see @fig-InsertingImage).

![](images/Quarto_Insert_image.png){#fig-InsertingImage fig-alt="Cartoon drawing of a pipe with three entry points for \"data\" and the output producing a research paper with text, a table, and a plot. Sections of the pipe are labelled: data cleaning, overview, figures, modelling, and text." width="480"}
...
-   [ ] Define concepts of literate programming and reproducible research
-   [ ] Install Quarto, open a new Quarto document in RStudio and write and format text in Quarto (bold, first-level heading, italics, etc.)
-   [ ] Insert a code chunk in a Quarto document and use inline codes
-   [ ] Construct a table in a Quarto document and embed images and plots
