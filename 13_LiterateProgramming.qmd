...
# Writing and publishing in Quarto {#sec-LiteRateProgramming}
...
#### Click here for the solution to [**Task 13.1**]{style="color:green;"}
...
The rendered version should read like this (if you are obtaining different numbers, this either means that you have tempered with the original data files or that they have been corrupted)[^13_literateprogramming-6]:
...
::: {.callout-note collapse="true"}
#### Click here for the solution to [**Task 13.2**]{style="color:green;"}
...
If you want to share a single, self-contained `.html` file with someone else or upload it somewhere, you will need to embed all the necessary files directly inside your HTML file. This is achieved by adding the following option to your Quarto document's YAML header:
...
With this setting, Quarto will package all necessary resources inside the HTML file, giving you a self-contained document that is easy to share and archive as it can be viewed in any web browser (e.g., Firefox, Google Chrome, Safari).
...
By default, the quality of the images and graphs in rendered `.docx` versions is low to keep the file size small. You can improve the quality of the rendered images by specifying the image definition in the YAML option. Make sure that you indent each line correctly as shown below; otherwise, you will get an error when you try to render your document:
...
Note that dynamic code outputs (such as the interactive {plotly} plot displayed in @fig-scatterplot-plotly) cannot meaningfully be rendered to static formats such as Microsoft Word documents or PDF. Attempting to do so can cause rendering errors. To avoid `R` attempting to render these code chunks to formats other than HTML, you can enclose them in the following div block:
...
To share your work with **LibreOffice**, **OnlyOffice**, and **OpenOffice** users, use the `format: odt` option, which will generate an **OpenDocument** - an open standard file format that can be opened in any text-processing software.
...
:::: {.callout-note collapse="true"}
#### Click here for the solution to [**Task 13.4**]{style="color:green;"}
