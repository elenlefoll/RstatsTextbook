...
So far, we have seen how we can export the outputs of our analyses conducted in `R`, both in the form of tables (@sec-ExportingRObjects) and graphics (@sec-ExploringPlots). However, in most situations, we want to communicate our research with the form of a publication that combines both text and analysis outputs. This is where literate programming comes into play!
...
-   sharing and publishing your research in different formats including:
    -   `.html`
    -   `.pdf`
    -   `.odt`
    -   `.docx`
...
3.  In RStudio, open a new Quarto document by selecting *File \> New File \> Quarto Document...* in the top menu, or clicking on the "new document" button. A dialogue menu will appear (Fig \*\*). Leave everything as is and simply click on "Create" at the bottom.\
    ![](images/Quarto_new.png){width="401"}

4.  RStudio has now opened a new, untitled Quarto file (`.qmd`). Change the title of the document (which is not the same as its filename!) and add the following three lines in the document header (the **YALM**) by copying and pasting the following lines at the top of the document (see Fig. \*\*).[^13_literateprogramming-4]
...
[^13_literateprogramming-4]: Note that, in the YALM, character strings such as the title and your name must be enclosed in quotation marks. The date is not enclosed in quotation marks because it is a dynamic variable that will be adjusted to your computer's system date so that, every time you render the document, the date will be updated.

::: {layout-ncol="2"}
![The `.qmd` file as opened in RStudio](images/LearningQuarto_qmd.png)

![The `.html` file as opened in a web browser](images/LearningQuarto_html.png)
:::
...
-   In a new line beginning after the final `---` of the YALM header, paste the introduction text below.
-   Using the Quarto editing toolbar, format the text so that, in the Visual mode, it looks like the text displayed in Fig. \*\*.
...
![](images/LearningQuarto_intro.png)
:::

::: {.callout-note collapse="true"}
#### Click here for the solution to Task 13.1

In the Visual mode (see Fig. \*\* a), you will need to click on the "Normal" drop-down menu (see Fig. \*\* b) to change the formatting of the word *Introduction* to the "Header 1" style.

To format the long citation, choose the "Blockquote" option from the the "Format" drop-down menu (see Fig. \*\* c).

![](images/Quarto_visual.png){width="442"}
:::

## Markdown text {#sec-Markdown}

As you discovered in Task 6.1, writing and formatting text in RStudio's Visual editor is very similar to writing in a word-processing software such as LibreOffice Writer or Microsoft Word. In the background, however, RStudio converts all formatting to **Markdown**.
...
Q13.7 How will the word `_mystery_` be formatted in Markdown?
...
Chunk options are placed at the top of the corresponding chunk following `#|`, as in the chunk below. As you can see, the `eval: false` chunk option means that the rendered document includes the mathematical operation `13 * 13`, but not the result because chunk was not executed during the rendering process.
...
a\) What is the output of the `setup` chunk in your rendered `.html` document?
...
b\) Which code chunk option can you use to remove the two messages from the rendered version of your Quarto document, whilst still ensuring that the `setup` chunk is displayed and executed (so that the libraries can be used in future code chunks)?
...
c\) Which code chunk option can you use to remove both the `setup` chunk and its outputs from the rendered version of your Quarto document, whilst still ensuring that the libraries are loaded so that their functions can be used further down in the document?
...
Q13. Compare the rendered version of your document with the original descriptive statistics reported in Dąbrowska [@DabrowskaExperienceAptitudeIndividual2019: 6]. Could you successfully **reproduce** these descriptive statistics? Which values are different?
...
To embed an image from an external file, you can use the "Insert" menu in RStudio's **Visual** editor and select "Figure / Image" (see Fig. \*\*). This will open up a menu where you can select the image that you want to insert, as well as add **alt-text** (see @sec-Alt-text) and a **caption**. The easiest way to adjust the size of an embedded image is to click on the image and then adjust the **size** of the image with the blue circle in the bottom-right corner of the image (see Fig. \*\*).

![](images/Quarto_Insert_image.png){fig-alt="Cartoon drawing of a pipe with three entry points for \"data\" and the output producing a research paper with text, a table, and a plot. Sections of the pipe are labelled: data cleaning, overview, figures, modelling, and text." width="480"}
...
#| warning: false
#| code-fold: true
#| code-summary: "Show `R` code to generate interactive plot below."
...
In order to connect this `bibliography.bib` file with our Quarto document, we need to add a `bibliography` key to our **YALM header**. If our `references.bib` file is located in the same folder as our Quarto document (which is what RStudio does by default), we can simply add the following line to our YALM:
...
With this modified YALM, when the document is rendered, a bibliography will automatically be added to the end of the document. This means that, if you have citations in your document, it is a good idea to include a header section `# References` at the end of the document.
...
[**b.**]{style="color:green;"} How are the authors' names listed in the bibliography formatted in APA7?
...
You will first need to add a reference to the BibTeX file generated by {grateful} in your YAML header. This means that your Quarto document will now have two bibliography files, which is fine as long as you use the following YALM syntax to reference them both.
...
Alternatively, `cite_packages()` can generate a table with all the package names, versions, and references. Table \*\* lists all of the packages used in the making of this textbook.
...
The {renv} library keeps track of the exact package versions that your project depends on, and ensures that those exact versions are installed whenever and wherever your project is opened. {renv} provides each project with its own isolated package library, ensuring that you can update packages in new projects without risking breaking older projects. It therefore helps make your projects more isolated, portable and reproducible.
...
:::: {.callout-note collapse="true"}
#### Click here for the solution to Task 13.\*\*

Below is Quarto document with the inline code chunks and its rendered `.odt` version as opened in LibreOffice Writer.
...
This is likely to take several minutes but you will only need to do it once. Afterwards, you can add the following line to your Quarto YALM header and you're ready to render to PDF!
...
Quarto's different publishing formats have different options. For example, if you want your PDF document have numbered sections and to include a table of content (TOC), you can add the following lines to your YALM header:
...
|  |  |  |
|----|----|----|
| [Revealjs](/docs/reference/formats/presentations/revealjs.qmd) | Revealjs is an open-source HTML presentation framework. | `format: revealjs` |
| [PowerPoint](/docs/reference/formats/presentations/pptx.qmd) | PowerPoint is Microsoft Office's presentation editing software. | `format: pptx` |
| [Beamer](/docs/reference/formats/presentations/beamer.qmd) | Beamer is a LaTeX class for producing presentations and slides in PDF format. | `format: beamer` |
