...
[**Q13.7**]{style="color:green;"} How will the word `~~mystery~~` be formatted in Markdown?
...
-   the entire code chunk either by clicking the "Run"![](https://d33wubrfki0l68.cloudfront.net/18153fb9953057ee5cff086122bd26f9cee8fe93/3aba9/images/notebook-run-chunk.png)icon or using the shortcut ⇧ ⌘/Ctrl ⏎.
...
If we want to start our paragraph with 90 written in as a word rather than in digits, we can use the `numbers_to_words function()` function from the {xfun} package. First, you'll need to install the {xfun} package and then add a line to your `setup` chunk to load it.[^13_literateprogramming-7]

[^13_literateprogramming-7]: To make your Quarto document even more reproducible, consider replacing your `set-up` chunk with the following function that will automatically check if a package needs to be installed before it is loaded.
...
Below is the source code for @fig-RealisticPipeline. The code includes the **relative path** to the image file (see @sec-FoldersPaths) relative to the **project directory** (see @sec-RProject). In the example below, the image file `BERD_pipeline-real.jpg` is located in a subfolder called `images`.
