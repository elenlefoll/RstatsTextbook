...
+----------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+-------------------------------------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| **Stratifiedsampling\            | The population is divided into strata (e.g., by dialectal region, age group, education level) and a random sample is drawn from each stratum. | Used when researchers need balanced data from distinct sub‑populations (e.g., speakers from several dialectal areas) and their intersections (e.g., a good balance of genders across each dialectal area). | Guarantees coverage of all relevant sub‑populations; reduces sampling error within the strata.  | As above, requires reliable, exhaustive lists for each stratum and consent for all selected; more complex to organise.                                                       |
...
[**Optional task**]{style="color:green;"} 🦖

Inspired by the Anscombe Quartet, @matejkaSameStatsDifferent2017 created a set of 12 pairs of variables that have the same descriptive statistics as the data that produce a scatter plot representing a tyrannosaurus[^11_inferential-7] (see @fig-DinoPLot).
...
#| fig-height: 6
#| label: "fig-DinoPLot"
#| fig-cap: "Scatter plot depicting a tyrannosaurus (originally created by Alberto Cairo)"
...
[^11_inferential-7]: This scatter plot was first designed by [Alberto Cairo](https://web.archive.org/web/20240620205540/http://www.thefunctionalart.com/2016/08/download-datasaurus-never-trust-summary.html).
...
-   Check that the main underlying assumptions of statistical tests are met:
    -   [ ] randomisation (@sec-Randomisation)
    -   [ ] independence (@sec-Independence)
    -   [ ] normality (@sec-Normality)
