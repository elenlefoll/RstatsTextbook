...
-   A correlation coefficient of **1** means that there is a **perfect positive linear relationship** between the two variables.
    ...
    -   [Example]{.underline}: The relationship between the number of questions that a person correctly answered in a test and the percentage of questions that they got right.
        ...
        # First, let's imagine that 100 learners took a test with 15 questions. We simulate the number of questions that they answered correctly using the `sample()` function that generates random numbers, specifying that learners can get between zero and 15 questions right:
        correct.answers <- sample(0:15, 100, replace = TRUE)

        # Next, we convert the number of correctly answered questions into the percentage of questions that they answered correctly:
        accuracy <- correct.answers / 15 * 100
...
The dataset from @DabrowskaExperienceAptitudeIndividual2019 contain the results of numerous additional tests that we have not yet examined.
...
check_question("19%",
               ...
               wrong = "❌")
check_hint("Section 11.7 showed how to calculate this risk for 20 individual tests. Can you adapt the formula for four tests?", 
           hint_title = "🐭 Click on the mouse for a hint.")
```
